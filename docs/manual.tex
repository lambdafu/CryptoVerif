...
$\nonterm{pest}$ is an identifier that estimates a probability:
\texttt{pest$n$} for estimate $n$, corresponding to probability $2^{-n}$,
\texttt{password} for estimate 20, i.e. probability $2^{-20}$,
\texttt{large} for estimate 256, i.e. probability $2^{-256}$. (See also the declarations \texttt{param}, \texttt{proba},
...
and $\texttt{Pcoll1rand}(T)$ is at most the probability estimated by $\nonterm{pest}$.
By default, collisions are eliminated for $\textit{anything} \times \texttt{Pcoll1rand}(T)$ when $\texttt{Pcoll1rand}(T) \leq 2^{-256}$ (\texttt{large}),
...
\item \texttt{allowed\string_collisions} $\nonterm{pest}$, where $\nonterm{pest}$ is an identifier that estimates a probability, as above. Collisions are eliminated when the probability that they generate, taking into account how many times they are applied, is at most the probability specified by $\nonterm{pest}$. This behavior fits the exact security framework nicely: we eliminate collisions when they have a small enough probability.
