...
\newcommand{\modop}{\mathbin{\mathrm{mod}}}%
   More generally, it supports a curve defined by the equation
   ...
   $k = 8$, $k' = 4$, $q = 2^{252} + \delta$ with $0 < \delta < 2^{128}$,
    ...
    $n_{min} = 2^{251}$, and $n_{max} = 2^{252}-1$.
       ...
       $Z$: type of exponents (must be \texttt{bounded}, \texttt{nonuniform}, and \texttt{large}). 
       $Z$ is the set of integers multiple of $k$, prime to $qq'$, modulo $kqq'$. Random choices in $Z$ are done by choosing uniformly in 
   $\{ kn \mid n \in [n_{min},n_{max}] \}$, hence $\texttt{Pcoll1rand}(Z) = 1/(n_{max}-n_{min}+1)$. (For curve25519, $\texttt{Pcoll1rand}(Z) = 2^{-251}$.)
       
       $g$: represents the base point.

       $\vn{exp}(G, Z): G$: the exponentiation function.  
       
       $\vn{mult}(Z, Z): Z$: the multiplication function for exponents, defined as
       $\vn{mult}(x,y) = x.y \mod kqq'$. (It remains in $Z$.)
