  ...
  Diffie-Hellman assumption; $p(t)$ is the probability of breaking the CDH assumption, for one pair of exponents, in time $t$.
   ...
   \texttt{crypto} command (see Section~\ref{sec:interact}).
     ...
     \item $\texttt{expand\ CDH\_RSR(}G, Z, g, \vn{exp}, \vn{exp}', \vn{mult}, p, p_d\texttt{).}$ 
       ...
       $p_d = 2^{-125}$ for Curve25519, and $p_d = 2^{-220}$ for Curve448.
...
Warning: if $G$ is Curve25519 or Curve448, this assumption must be used together
with the precise model of Curve25519/448 defined in the macros \texttt{DH\_X25519},
\texttt{DH\_X448}, or \texttt{DH\_single\_coord\_ladder}, and \texttt{PRF\_ODH2}
must be applied to the subgroup, e.g.
\begin{align*}
  ...
  &\texttt{expand\ PRF\_ODH2(}\vn{subG}, \vn{Znw}, \vn{prf\_in}, \vn{prf\_out},
...
\end{align*}
(The existence of equivalent public keys in whole Curve25519/448 breaks \texttt{PRF\_ODH2}.)
...
Warning: if $G$ is Curve25519 or Curve448, this assumption must be used together
with the precise model of Curve25519/448 defined in the macros \texttt{DH\_X25519},
\texttt{DH\_X448}, or \texttt{DH\_single\_coord\_ladder}, and \texttt{square\_PRF\_ODH2}
must be applied to the subgroup, e.g.
\begin{align*}
  &\texttt{expand\ DH\_single\_coord\_ladder}(G, Z, g, \vn{exp}, \vn{mult}, \vn{subG}, \vn{Znw}, \vn{ZnwtoZ}, \vn{g\_k},\\
  &\qquad\vn{exp\_div\_k}, \vn{exp\_div\_k}', \vn{pow\_k}, \vn{subGtoG}, \vn{zero}, \vn{sub\_zero}\texttt{).}\\
  &\texttt{expand\ square\_PRF\_ODH2(}\vn{subG}, \vn{Znw}, \vn{prf\_in}, \vn{prf\_out},
  \vn{g\_k}, \vn{exp\_div\_k}, \vn{exp\_div\_k}', \\
  &\qquad\vn{mult}, \vn{prf\_subG}, p, \vn{sqp}\texttt{).}\\
  &\texttt{fun}\ \vn{prf}(G, \vn{prf\_in}): \vn{prf\_out}.\\
  &\texttt{equation\ forall}\ x:\vn{subG}, y:\vn{prf\_in}; \vn{prf}(\vn{subGtoG}(x),y) = \vn{prf\_subG}(x,y).
\end{align*}
(The existence of equivalent public keys in whole Curve25519/448 breaks \texttt{square\_PRF\_ODH2}.)
     ...
     \item $\texttt{expand\ CDH\_single(}G, Z, g, \vn{exp}, \vn{exp}', \vn{mult}, p\texttt{).}$\\
       ...
       $\texttt{expand\ PRF\_ODH2\_single(}G, Z, \vn{prf\_in}, \vn{prf\_out}, g, \vn{exp}, \vn{exp}', \vn{mult}, \vn{prf}, p\texttt{).}$\\
...
\item $\texttt{guess }i$ guesses the tested session, where the identifier $i$ is the
replication index to be guessed; there must be a single
replication with index $i$ in the game. Alternatively, the form $\texttt{guess }occ$ can also be used,
where $occ$ is the occurrence of the replication whose index $i$ is to be guessed. 
CryptoVerif distinguishes whether the replication index $i$ 
is equal to a constant value $i_{\mathit{tested}}$ by introducing a test 
under the considered replication, and tries to prove the
security properties for the session in which $i = i_{\mathit{tested}}$.
The queries are adjusted accordingly. The probability of breaking
the initial query is typically $N$ times the probability of
breaking the query for session $i = i_{\mathit{tested}}$, where
the replication index $i$ is in $[1,N]$.

This transformation cannot apply to injective correspondences.
If injective correspondences are present, they are left unchanged
and proved on all sessions. If all queries are injective
correspondences, the transformation fails.

This transformation does not apply to $\texttt{equivalence}$
and $\texttt{query\_equiv}$ proofs either. The transformation fails
