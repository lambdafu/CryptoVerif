...
\item $\texttt{expand IND\_CPA\_INT\_CTXT\_sym\_enc(}\vn{keyseed}, \vn{key},
  ...
  ciphertexts of length at most $l'$.
   ...
   (see Section~\ref{sec:interact}). The equivalence $\texttt{int\_ctxt}(\vn{enc})$
   should generally be applied before $\texttt{ind\_cpa}(\vn{enc})$,
   because $\texttt{int\_ctxt}(\vn{enc})$ eliminates the decryption oracle.

\item $\texttt{expand IND\_CCA2\_sym\_enc(}\vn{keyseed}, \vn{key},
  \vn{cleartext}, \vn{ciphertext}, \vn{seed}, \vn{kgen}, \vn{enc},
  \vn{dec}, \vn{injbot}, \ab \vn{Z}, \ab \vn{Penc}\texttt{).}$ defines a
  ...
  ciphertexts of length at most $l'$.
   ...
   This macro defines the equivalence named $\texttt{ind\_cca2}(\vn{enc})$, for use in the \texttt{crypto} command 
   (see Section~\ref{sec:interact}). 

\item $\texttt{expand INT\_PTXT\_sym\_enc(}\vn{keyseed}, \vn{key},
  \vn{cleartext}, \vn{ciphertext}, \vn{seed}, \vn{kgen}, \vn{enc},
  \vn{dec}, \vn{injbot}, \ab \vn{Pencptxt}\texttt{).}$ defines an INT-PTXT (plaintext integrity)
  probabilistic symmetric encryption scheme.
  ...
  ciphertexts of length at most $l'$.
   ...
   The types $\vn{keyseed}$, $\vn{key}$, $\vn{cleartext}$,
   $\vn{ciphertext}$, $\vn{seed}$ and the probability $\vn{Pencptxt}$ must
   be declared before this macro is expanded. The functions
   $\vn{kgen}$, $\vn{enc}$, $\vn{dec}$, and $\vn{injbot}$ are declared by this
   macro. They must not be declared elsewhere, and they can be used
   only after expanding the macro.

   This macro defines the equivalences named
   $\texttt{int\_ptxt}(\vn{enc})$, for use in the \texttt{crypto} command 
   (see Section~\ref{sec:interact}). 
...
\item $\texttt{expand IND\_CCA2\_INT\_PTXT\_sym\_enc(}\vn{keyseed}, \vn{key},
  ...
  ciphertexts of length at most $l'$.
   ...
   and $\texttt{int\_ptxt\_after\_ind\_cca2}(\vn{enc})$, for use in the \texttt{crypto} command 
   (see Section~\ref{sec:interact}). The first two correspond to the IND-CCA2 property,
   the last two to the INT-PTXT property. The equivalence $\texttt{ind\_cca2}(\vn{enc})$
   can be applied before applying the INT-PTXT property, while $\texttt{ind\_cca2\_after\_int\_ptxt}(\vn{enc})$
   can be applied after applying the INT-PTXT property. Similarly, the equivalence  $\texttt{int\_ptxt}(\vn{enc})$
   can be applied before applying the IND-CCA2 property, while $\texttt{int\_ptxt\_after\_ind\_cca2}(\vn{enc})$
   can be applied after applying the IND-CCA2 property.
...
\item $\texttt{expand ICM\_cipher(}\vn{cipherkey}, \vn{key}, \vn{blocksize}, \vn{enc},
   \vn{dec}%, \vn{enc\_dec\_oracle}, \vn{qE}, \vn{qD}
\texttt{).}$
...
%    $\vn{enc\_dec\_oracle}$ is a process that allows the adversary to
%    call the encryption and decryption functions.
%    WARNING: the encryption and decryption oracles must be made
%    available to the adversary, by including $\vn{enc\_dec\_oracle}$ in
%    the executed process.

%    $\vn{qE}$ is the number of requests to the encryption oracle.

%    $\vn{qD}$ is the number of requests to the decryption oracle.
...
\item $\texttt{expand IND\_CCA2\_public\_key\_enc(}\vn{keyseed}, \vn{pkey}, \vn{skey},
  ...
  probabilistic public-key encryption scheme.
   ...
   $\vn{enc}(\vn{cleartext}, \vn{pkey}, \vn{seed}): \vn{ciphertext}$ is the encryption function.
  ...
  injection from $\vn{cleartext}$ to \texttt{bitstringbot}.
   ...
   $\vn{Z}: \vn{cleartext}$ is a constant cleartext. The encryption scheme is assumed to
   encrypt a block, so that the length of the cleartext is not leaked.
   ...
   $\vn{cleartext}$, $\vn{ciphertext}$, $\vn{seed}$ and the
   ...
   This macro defines the equivalence named $\texttt{ind\_cca2}(\vn{enc})$
   for use in the \texttt{crypto} command 
   (see Section~\ref{sec:interact}).

\item $\texttt{expand UF\_CMA\_signature(}\vn{keyseed}, \vn{pkey}, \vn{skey},
  \vn{signinput}, \vn{signature}, \vn{seed}, \vn{skgen}, \vn{pkgen}, \ab \vn{sign}, \ab
  \vn{check}, \ab \vn{Psign}, \ab\vn{Psigncoll}\texttt{).}$ defines a
  ...
  $\vn{signinput}$ is the type of signature inputs.
   ...
   $\vn{seed}$ is the type of random seeds for signatures, must be \texttt{bounded}, typically \texttt{fixed}.

   $\vn{skgen}(\vn{keyseed}): \vn{skey}$ is the secret key generation function.

   $\vn{pkgen}(\vn{keyseed}): \vn{pkey}$ is the public key generation function.

   $\vn{sign}(\vn{signinput}, \vn{skey}, \vn{seed}): \vn{signature}$ is the signature function.
  ...
  verification function.
   ...
   $\vn{signature}$, $\vn{seed}$ and the probabilities $\vn{Psign}$, $\vn{Psigncoll}$ must
   be declared before this macro is expanded. The functions
   ...
   This macro defines the equivalences named
   $\texttt{uf\_cma}(\vn{sign})$ and
   $\texttt{uf\_cma\_corrupt}(\vn{sign})$, for use in the
   \texttt{crypto} command (see Section~\ref{sec:interact}).
   Both equivalences correspond to the UF-CMA property, but the former 
   does not allow corruption of the secret keys while latter allows it.
   The latter equivalence is applied only manually, because its automatic
...
\item $\texttt{expand SUF\_CMA\_signature(}\vn{keyseed}, \vn{pkey}, \vn{skey},
  \vn{signinput}, \vn{signature}, \vn{seed}, \vn{skgen}, \vn{pkgen}, \ab \vn{sign}, \ab
  \vn{check}, \ab \vn{Psign}, \ab\vn{Psigncoll}\texttt{).}$ defines a
  ...
  signature. The arguments are the same as for the previous macro.
   This macro defines the equivalences named
   $\texttt{suf\_cma}(\vn{sign})$ and
   $\texttt{suf\_cma\_corrupt}(\vn{sign})$, for use in the
   \texttt{crypto} command (see Section~\ref{sec:interact}).

\item $\texttt{expand ROM\_hash(}\vn{key}, \vn{hashinput}, \vn{hashoutput}, \vn{hash}%, \vn{hashoracle}, \vn{qH}
\texttt{).}$
...
\texttt{fixed}.
   ...
   by including a process such as
      \texttt{!qH in(c1, x:$\vn{hashinput}$); out(c2, hash(k,x))}
...
%    $\vn{qH}$ is the number of requests to the hash oracle
 ...
 \item
   $\texttt{expand ROM\_hash\_pair(}\vn{key}, \vn{hashinput1}, \vn{hashinput2}, \vn{hashoutput}, \vn{hash}%, \vn{hashoracle}, \vn{qH}
   \texttt{).}$\\
   $\texttt{expand ROM\_hash\_triple(}\vn{key}, \vn{hashinput1}, \vn{hashinput2}, \vn{hashinput3}, \vn{hashoutput}, \vn{hash}%, \vn{hashoracle}, \vn{qH}\
   \texttt{).}$\\
$\texttt{expand ROM\_hash\_quad(}\vn{key}, \vn{hashinput1}, \vn{hashinput2}, \vn{hashinput3}, \vn{hashinput4}, \vn{hashoutput}, \vn{hash}%, \vn{hashoracle}, \vn{qH}
   \texttt{).}$\\
   define random oracle with 2, 3, and 4 arguments respectively, similarly to \texttt{ROM\_hash} above.
   Letting $N$ be the number of arguments of the oracle,
   $\vn{hashinput1},\dots,\vn{hashinput}N$ are the types of the inputs of the hash function,
   ...
   \texttt{! qH in(c1, (x1:$\vn{hashinput1}$,$\dots$,x$N$:$\vn{hashinput}N$)); out(c2, hash(k,x1,...,x$N$))}.
...
\item $\texttt{expand\ GDH(}G, Z, g, \vn{exp}, \vn{mult},
  \vn{PGDH}\texttt{).}$ defines a group that satisfies the gap
  Diffie-Hellman assumption. The arguments are the same as for
  \texttt{CDH} above, except that the probability $\vn{PCDH}$ is
  replaced with $\vn{PGDH}$. This macro defines the same equivalences
as the \texttt{CDH} macro, except that $\texttt{cdh}(\vn{exp})$ is replaced
with $\texttt{gdh}(\vn{exp})$.

\item $\texttt{expand\ GDH\_prime\_order(}G, Z, g, \vn{exp}, \vn{mult},
  \vn{PGDH}\texttt{).}$ defines a group that satisfies the gap
  Diffie-Hellman assumption and has prime order. The interface is exactly
...
\item $\texttt{binder }x$: move random number generations and assignments
that define $x$ (even when this is not beneficial).
...
\item $\texttt{simplify coll\string_elim }x_1 \ldots x_n$: simplify the game,
...
the program, variable names (meaning all occurrences of these
variables), or types (meaning all data of those types).
If one wants to specify occurrence numbers, one should use the command 
\texttt{show\string_game occ} to determine which occurrence numbers
correspond to the desired program points.
...
macro expansion adds a prefix \texttt{\string@$k$\_} to the variable
and oracle names of the equivalence. This prefix must be included in
...
prefix can be determined by issuing a command \texttt{crypto} without
...
then tries to bound the probability of executing that event.
