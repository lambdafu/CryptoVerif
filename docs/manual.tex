...
defines a pseudo-random function.
 ...
 \item The specification of Diffie-Hellman key agreements is typically composed of two macro expansions:
   ...
   \item
     ...
     Z, g, \vn{exp}, \vn{exp}', \vn{mult}, \vn{PDist}\texttt{).}$ says
     ...
     already includes these properties with $\vn{PDist} = 0$.
 ...
 \item Another possible set of macros for Diffie-Hellman assumptions is the following.
   \begin{itemize}
   \item $\texttt{expand\ DH\_prime\_subgroup\_secret\_keys\_not\_multiple\_k}$

   \item 

   \end{itemize}
  
   $\texttt{expand\ CDH(}G, Z, g, \vn{exp}, \vn{mult},
