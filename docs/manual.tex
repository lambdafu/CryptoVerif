 ...
 \item The specification of Diffie-Hellman key agreements is typically composed of two or three macro expansions:
   ...
   random $x$ and with $Y$ independent of $x$ is at most $\vn{PCollKey3}$, with $\vn{PCollKey3} \geq \vn{PCollKey2}$.
     ...
     \item $\texttt{expand\ DH\_good\_group(}G, Z, g, \vn{exp}, \vn{exp}', \vn{mult}\texttt{).}$ defines a group $G$ like
       ...
       $\vn{mult}$ is the product modulo $q$ in $\{1, ..., q-1\}$, i.e. in the group $(\mathbb{Z}/q\mathbb{Z})*$,
     ...
     \item $\texttt{expand\ DH\_prime\_subgroup\_secret\_keys\_multiple\_k}(G$, $Z$, $g$, $\vn{exp}$, $\vn{mult}$, $\vn{subG}$, $\vn{g\_k}$, $\vn{exp\_div\_k}$, $\vn{exp\_div\_k}'$, $\vn{pow\_k}$, $\vn{subGtoG}$, $\vn{zero}$, $\vn{sub\_zero}\texttt{).}$ defines a Diffie-Hellman group with the following properties.
       ...
       $\vn{mult}(Z, Z): Z$: the multiplication function for exponents, defined as
       $\vn{mult}(x,y) = x.y \mod kq$. (It remains multiple of $k$.)
       ...
       $\vn{pow\_k}(G):\vn{subG}$, defined by $\vn{pow\_k}(x) = x^k$.

       $\vn{subGtoG}(\vn{subG}):G$ is the natural injection from subG to G.

       $\vn{zero}$ is the neutral element of $G$.
       
       $\vn{sub\_zero}$ is $\vn{zero}$, but as an element of $\vn{subG}$.

       The types $G$, $\vn{subG}$, and $Z$ must be declared before this macro.  The
       functions $g$, $\vn{exp}$, $\vn{mult}$, $\vn{g\_k}$, $\vn{exp\_div\_k}$, $\vn{exp\_div\_k}'$, $\vn{pow\_k}$, $\vn{subGtoG}$,
       ...
       $\texttt{expand\ }\vn{assumption}\texttt{(}\vn{subG}$, $Z$, $\vn{g\_k}$, $\vn{exp\_div\_k}$, $\vn{exp\_div\_k}'$, $\vn{mult}$, $\dots\texttt{).}$

       For instance, these properties are satisfied by Curve25519 when
       all secret keys are multiple of 8, which is true if RFC 7748 is
       followed (\url{https://tools.ietf.org/html/rfc7748}). In this
       case, $k = 8$ and the choice of a secret key is non uniform in
       $Z$: secret keys are chosen uniformly among $2^{254} + 8n$
       ($n \in [0, 2^{251}-1]$), that is, $2^{251}$ elements. So
       the type $Z$ should be declared \texttt{nonuniform}
       and we have $\texttt{Pcoll1rand}(Z) = 1/2^{251}$.
   ...
   \end{itemize}
     ...
     already includes these properties with $\vn{PDist} = 0$. when the macro $\texttt{expand\ DH\_prime\_subgroup\_secret\_keys\_multiple\_k}(G$, $Z$, $g$, $\vn{exp}$, $\vn{mult}$, $\vn{subG}$, $\vn{g\_k}$, $\vn{exp\_div\_k}$, $\vn{exp\_div\_k}'$, $\vn{pow\_k}$, $\vn{subGtoG}$, $\vn{zero}$, $\vn{sub\_zero}\texttt{).}$ is used, this macro should be applied to the subgroup:
     $\texttt{expand\ DH\_dist\_random\_group\_element\_vs\_exponent(}\vn{subG}$, $Z$, $\vn{g\_k}$, $\vn{exp\_div\_k}$, $\vn{exp\_div\_k}'$, $\vn{mult}$, $\vn{Pdist}\texttt{).}$
...
\vn{partN\_t}, \vn{concat}, \vn{reformat}\texttt{).}$ defines allows to split a random 
value into $N$ values, for $N \leq 10$.
  ...
  $\vn{concat}(\vn{part1\_t}, ..., \vn{partN\_t}): \vn{input\_t}$: function that takes 
  $N$ parts as input and returns the corresponding value of type $\vn{input\_t}$
  by concatenating the parts.

  $\vn{reformat}(\vn{input\_t}): \vn{input\_t}$ reformats the input so
  that it is suitable for a pattern-matching with
  $\vn{concat}(\dots)$.  This function is the identity in CryptoVerif
  but not in ProVerif. In ProVerif, pattern-matching a random value or
  the result of a hash function with $\vn{concat}(\dots)$ would always
  fail.

  Usage to split a value $y$ of type $\vn{input\_t}$ into $N$ parts 
  of types $\vn{part1\_t}, \dots, \vn{partN\_t}$:
\[\texttt{let}\ \vn{concat}(x1, ..., xN) = \vn{reformat}(y)\ \texttt{in}\ \dots\]
  or in CryptoVerif only:
\[\texttt{let}\ \vn{concat}(x1, ..., xN) = y\ \texttt{in}\ \dots\]
In CryptoVerif, this macro defines the equivalence named
$\texttt{splitter}(\vn{concat})$ which replaces a random number
generation in $\vn{input\_t}$ with the concatenation of $N$
independent random number generations in
$\vn{part1\_t}, \dots, \vn{partN\_t}$.

$\vn{input\_t}$, $\vn{part1\_t}, \dots, \vn{partN\_t}$ must be defined
before expanding this macro.  $\vn{concat}$ and $\vn{reformat}$ are
defined by this macro. They must not be declared elsewhere, and they
can be used only after expanding the macro.
