...
\item $\texttt{guess }\nonterm{guessspec}$ guesses value of $\nonterm{guessspec}$ in the tested session,
  where $\nonterm{guessspec}$ can be one of the following:
  \begin{enumerate}
  \item $i$, where $i$ is the replication index to be guessed. There must be a single
    replication with index $i$ in the game.

  \item $occ$, where $occ$ is the occurrence of the replication whose index $i$ is to be guessed.

  \item $i\texttt{ \&\& above}$, where $i$ is a replication index; then, just under the replication with index $i$,
    one guesses the whole sequence of replication indices above the replication with index $i$.
    There must be a single replication with index $i$ in the game.

  \item $occ\texttt{ \&\& above}$, where $occ$ is the occurrence of a replication; then, just under that replication,
    one guesses the whole sequence of replication indices above that replication.

  \item $\texttt{"}x[i_1, \dots, i_n]\texttt{"}$, where $x[i_1, \dots, i_n]$ is the variable to be guessed,
    defined under $n$ replications, and $i_1, \dots, i_n$ are constant replication indices
    (typically produced by a previous guess of the replication indices).
...
security properties for the session in which $\nonterm{guessspec} = v_{\mathit{tested}}$.
...
breaking the query for session $\nonterm{guessspec} = v_{\mathit{tested}}$.
...
\item $\texttt{out\string_commands}\ f$: output the executed interactive commands
  to file $f$. In no output file was specified before, outputs both the previous
  and future interactive commands to $f$. In an output file was specified before
