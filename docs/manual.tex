...
\item $\texttt{-impl }\nonterm{language}$: Instead of proving the protocol, generate an
  implementation in the chosen language corresponding to the modules defined in the input
  file. The language can be OCaml or FStar.
...
\texttt{fixed}.
   ...
   This macro defines equivalences named $\texttt{rom}(\vn{hash})$ and $\texttt{rom\_partial}(\vn{hash})$
   for use in the \texttt{crypto} command 
   (see Section~\ref{sec:interact}). These equivalences are generated via \texttt{equiv} \dots \texttt{special};
   the \texttt{crypto} command therefore supports special arguments $\mathit{collisions\_LHS}$ and, 
   for $\texttt{rom\_partial}(\vn{hash})$, collision matrix. See the explanation of the $\mathit{collisions\_LHS}$
   argument, the collision matrix, and the oracles present in these equivalences in the documentation of \texttt{equiv} \dots \texttt{special}.
...
\item $\texttt{expand UniversalOneWay\_hash(}\vn{key}, \vn{hashinput}, \vn{hashoutput}, \vn{hash}, \vn{hashoracle}, \vn{Phash}\texttt{).}$
defines a universal one-way hash function~\cite{Moni89}.
...
$\vn{hashoracle}$ is a process that leaks the key that it receives as argument.
   ...
   instance by including the process $\vn{hashoracle}(\vn{hk})$, where
   $\vn{hk}$ is the key. The key is often generated once and for all
  ...
  \item Similarly to the macros above, for $N$ from 1 to 10, the macros\\
   ...
   $\texttt{expand UniversalOneWay\_hash\_$N$(}\vn{key}$, $\vn{hashinput1}$, \dots, $\vn{hashinput}N$, $\vn{hashoutput}$, $\vn{hash}$, $\vn{hashoracle}$, $\vn{Phash}\texttt{).}$\\
...
\item $\texttt{move }m$: Try to move random number generations
  and assignments. It supports two versions.


  $\texttt{move\ up\ }x_1\ \dots\ x_n\texttt{ to }occ$ moves the
  random number generations or assignments of $x_1$, \dots, $x_n$ up
  in the syntax tree, to the program point $occ$. This program point
  must correspond to an output process resp. oracle body.
  After the game transformation, a variable $x$ (which may be among
  $x_1$, \dots, $x_n$) is defined at program point $occ$, and all
  other variables $x_i$ are defined by $\texttt{let }x_i = x\texttt{ in}$.
  All variables $x_1$, \dots, $x_n$ must have the same type.
  They must not be defined syntactically above the program point $occ$.
  Either all variables $x_1$, \dots, $x_n$ must be defined by
  random number generations or all of them must be defined by assignments.
  \begin{itemize}
  \item If $x_1$, \dots, $x_n$ are defined by random number
    generations, this transformation performs eager sampling of $x_i$.
    The random number generation of $x_1$, \dots $x_n$ must be executed at most
    once for each execution of program point $occ$.
  \item If $x_1$, \dots, $x_n$ are defined by assignments of terms $M_i$,
    then all $M_i$ must consist of variables, function applications,
    and tests; there must be one $M_i$ defined at program point $occ$
    (which will be used as the definition of $x$); and all terms $M_i$
    must be equal.
  \end{itemize}

For all other values of $m$, $\texttt{move }m$ moves random number generations
  and assignments down in the syntax tree:
\begin{itemize}

\item It moves random number generations down in the syntax tree as much
  as possible, in order to delay the choice of random numbers (lazy sampling).
  This is especially useful when the random number generations can be moved
...
\item It moves assignments down in the syntax tree but without duplicating
...
\end{itemize}
Only the random number generations and assignment at the process level
can be moved. (Those that are terms will be left unchanged.)
...
This transformation provides a stronger form of lazy sampling than
other variants of \texttt{move}.
...
\item $\texttt{SArename random}$: Renames all variables defined by \texttt{new}
...
\item \texttt{SArename random} if \texttt{autoSARename = true},
...
\item $\texttt{guess }\texttt{"}x[i_1, \dots, i_n]\texttt{" no\_test}$ guesses the value of $x[i_1, \dots, i_n]$, where
  $x[i_1, \dots, i_n]$ is defined under $n$
    replications, and $i_1, \dots, i_n$ are constant replication
    indices (typically produced by a previous guess of the replication
    indices).
    CryptoVerif must be able to determine, at each definition of $x$, whether its indices
    will be equal to $i_1, \dots, i_n$ or not. Otherwise, the transformation fails.
    When $x$ is defined under no replication, one writes $x$ instead of $\texttt{"}x[i_1, \dots, i_n]\texttt{"}$.
    The variable $x$ must be defined by $\texttt{let}\ x = \dots$

    CryptoVerif replaces the definition of $x$ with a constant value $v_{\mathit{tested}}$.
    When the definition of $x$ is not a simple term (a term built from replication indices, variables, and function applications), the old definition is still evaluated but ignored.

    Assuming $x$ is a variable of type $T$, the probability of
    breaking the initial query is $|T|$ times the probability of
    breaking the query for $x[i_1, \dots, i_n] = v_{\mathit{tested}}$.
    $|T|$ must be estimated less than \texttt{maxGuess} (see the
    command \texttt{set maxGuess}).

This transformation does not apply to secrecy queries, $\texttt{equivalence}$,
and $\texttt{query\_equiv}$ proofs. The transformation fails
if such a proof is still required.
...
\item $\texttt{guess\_branch }occ$ guesses which branch is taken at
  ...
  If this instruction has $n$ branches, CryptoVerif generates $n$ games $G_i$ ($0 \leq i < n$) in which branch $i$ is kept and all other branches are replaced with an event \texttt{bad\_guess}. (Branch 0 is the \texttt{else} branch.) The desired queries must then be proved in all games $G_i$ and the probability of breaking the queries in the initial game is bounded by the sum of the probabilities of breaking them in all games $G_i$.
...
\item $\texttt{guess\_branch }occ\texttt{ no\_test}$ guesses which branch is taken at
  program point $occ$. The instruction at program point $occ$ must be
  a test \texttt{if}. This instruction must be executed at most once (either because
  it is not under replication, and because it is under replication and the
  replication indices are fixed, for instance because they have previously
  been guessed by the \texttt{guess} instruction above).
  CryptoVerif generates 2 games $G_0$ and $G_1$, keeping respectively only the $\texttt{else}$ branch and the $\texttt{then}$ branch.
  The test is removed. If the condition of the test is not a simple term (a term built from replication indices, variables, and function applications), it is still evaluated but ignored.
  The desired queries must then be proved in both games $G_i$ and the probability of breaking the queries in the initial game is bounded by the sum of the probabilities of breaking them in both games $G_i$.
  That allows the user to split cases depending on which branch is taken at program point $occ$.
...
\item \texttt{move\_if\_fun\ }$\mathit{arg}$: moves the predefined function
  \texttt{if\_fun} or transforms it into a term \texttt{if} \dots \texttt{then} \dots \texttt{else} \dots\ It supports the following variants:
  \begin{itemize}
  \item \texttt{move\_if\_fun\ }$\mathit{loc}_1 \dots \mathit{loc}_n$, where each $\mathit{loc}_j$ is either a program point or a function symbol. When $\mathit{loc}_j$ is a program point, moves occurrences of \texttt{if\_fun} from inside the term at that program point to the root of that term. When $\mathit{loc}_j$ is a function symbol, moves occurrences of \texttt{if\_fun} from under that function symbol to just above it. 
  \item \texttt{move\_if\_fun\ level\ }$n$, where $n$ is a positive integer. Moves occurrences of \texttt{if\_fun} $n$ function symbols up in the syntax tree (provided those \texttt{if\_fun} occur under at least $n$ function symbols).
  \item \texttt{move\_if\_fun\ to\_term\ }$occ_1 \dots occ_n$ transforms the function symbols \texttt{if\_fun} that occur at program points $occ_1$, \dots, $occ_n$ into terms \texttt{if} \dots \texttt{then} \dots \texttt{else} \dots When no program point is given, performs that transformation everywhere in the game. When \texttt{autoExpand = true} (the default), a call to \texttt{expand} is automatically performed after \texttt{move\_if\_fun}, which transforms the terms \texttt{if} \dots \texttt{then} \dots \texttt{else} \dots into processes.
  \end{itemize}
...
By default, collisions are eliminated for $\textit{anything} \times \proba$ when $\proba \leq 2^{-155}$ (where \texttt{large} would mean $2^{-160}$, to allow collisions that have probability a small factor times collisions between elements of a \texttt{large} type),
and for $p \times \proba$ when $p \leq 2^2$ (\texttt{small}) and $\proba \leq 2^{-20}$ (\texttt{password}).
The default behavior is inspired by what happens in asymptotic security: \texttt{large} means that the probability of collision is asymptotically negligible, while the parameters are always polynomial, so $\textit{constant} \times p_1^{n_1} \times \dots \times p_k^{n_k} \times \texttt{Pcoll1rand}(T)$ is negligible when $T$ is \texttt{large}. Similarly, probabilities given in collision statements are always negligible (probabilities are declared \texttt{large} by default), while the parameters are always polynomial, so the probability obtained by applying $\textit{constant} \times p_1^{n_1} \times \dots \times p_k^{n_k}$ times a collision statement remains negligible.
...
\section{Generation of OCaml implementations}
\input {cv2OCaml.tex}

\section{Generation of F$^\star$ implementations}
\input {cv2fstar.tex}
