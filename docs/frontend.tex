...
{\tt defined}, {\tt else}, {\tt eps\_find}, {\tt eps\_rand}, {\tt equiv}, 
{\tt event}, {\tt expand}, {\tt find}, {\tt forall}, {\tt fun}, 
...
\else
{\tt abort}, {\tt collision}, {\tt const}, {\tt define}, 
{\tt defined}, {\tt do}, {\tt else}, {\tt end}, {\tt eps\_find}, 
{\tt eps\_rand}, 
{\tt equiv}, {\tt event}, {\tt expand}, 
...
\end{itemize}
\ifchannels
Terms contain further constructs \texttt{new}, \texttt{if}, \texttt{find},
\texttt{let} which are similar to the corresponding
...
The construct \texttt{new} is not allowed to occur in
conditions of \texttt{find}.
...
\texttt{let}, \texttt{if}, and \texttt{find} which are similar to the 
corresponding constructs of oracle bodies.
They are not allowed to occur in \texttt{defined} conditions of \texttt{find} and in input channels.
The construct \texttt{<-R} is not allowed to occur in
conditions of \texttt{find}.
We refer the reader to the description of 
...
Otherwise, it chooses randomly with uniform probability
...
\end{itemize}
The construct \texttt{event} in terms is also similar to the one for
processes, except that it is restricted to events without arguments
\texttt{event $f$} and that it aborts the process immediately after
the event. It is not allowed to occur in conditions of \texttt{find}
or in input channels. It is intended to be used in the right-hand side
...
\item {\tt abort} terminates the game. (Nothing can be executed after
...
Each such type $T$ comes with an associated standard probability distribution $D_T$;
...
\item When the type $T$ is {\tt nonuniform}, the standard probability 
...
time.
If $T$ is not marked {\tt nonuniform}, the standard probability 
...
the standard probability distribution $D_T$ is such that its distance
...
is chosen randomly with uniform probability. The communication is then
...
When a channel is just a channel name $c$, it is an abbreviation for
...
When {\tt true}, simplification tries to merge branches of {\tt if}, {\tt let},
...
\item \texttt{set mergeArrays = false.}\\
\texttt{set mergeArrays = true.}

When {\tt true}, simplification advises {\tt merge\_arrays} commands
to make the merging of branches of {\tt if}, {\tt find}, {\tt let}
succeed more often. When {\tt false}, this advice is not
automatically given and the user should use the manual commands
{\tt merge\_branches} and/or {\tt merge\_arrays} (defined in 
Section~\ref{sec:interact}) to perform the merging.

When it is set to {\tt true}, simplification often undoes
{\tt SArename}. That is why it is not set to {\tt true} by default.
...
When {\tt uniqueBranch = true} and some {\tt find}s are proved to be unique,
several transformations are enabled as part of {\tt simplify}:
\begin{itemize}
\item 
If a branch of a {\tt find} is proved to succeed and that {\tt find}
is unique, then simplification removes all other branches of that {\tt
find}.

\item
If a {\tt find} occurs in the {\tt then} branch of a {\tt find} 
and both {\tt find}s are unique, we reorganize them.

\item 
If a {\tt find} occurs in the condition of a {\tt find} and the
inner {\tt find} is unique, we reorganize them.

\end{itemize}
When {\tt uniqueBranch = false}, these transformations are not performed. 
...
chosen according to the standard probability distribution $D_T$ for the considered type $T$,
and an independent element of type $T$. When the standard distribution is uniform
...
\item \texttt{uniform} means that $f$ maps the standard distribution
of its argument into the standard distribution of its result. $f$ must be unary;
the argument and the result of $f$ must be of types marked 
{\tt fixed}, {\tt bounded}, or {\tt nonuniform}.

\item \texttt{commut} indicates that the function $f$ is commutative,
...
\fi
means that when
$x_1, \ldots, x_n$ are chosen randomly with uniform probability
and independently in $T_1, \ldots, T_n$ respectively, a Turing machine running in
...
\item ${\tt secret1}\ x$: show that any element of the array $x$ 
...
uniform probability distribution and the standard probability distribution 
...
to the standard distribution $D_T$ for type $T$ and an element of type $T$
...
$T$ in the standard distribution for that type:
...
chosen according to the standard distribution $D_T$ for type $T$
(when $T$ is \texttt{nonuniform}). We have
\[\texttt{Pcoll2rand(}T\texttt{)} = \sum_{a \in T} \Pr[X = a]^2 \leq \texttt{Pcoll1rand(}T\texttt{)}\]
where $X$ is chosen according to the standard distribution $D_T$.
...
a random number of type $T$ uniformly.
\item
\ifchannels
