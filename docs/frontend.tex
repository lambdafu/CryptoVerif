...
\texttt{pest$n$} for estimate $n$, where $n$ is an integer, means that the probability is about $2^{-n}$;
\texttt{password} for estimate 20, i.e. probability $2^{-20}$;
\texttt{large} for estimate 256, i.e. probability $2^{-256}$.
...
\texttt{size$n$} means that its cardinal is about $2^n$,
...
\item \texttt{large} is equivalent to \texttt{size256}, \texttt{pcoll256}
...
\item \texttt{password} is equivalent to \texttt{size20}, \texttt{pcoll20}.
...
assumes that password are taken in a dictionary of about one 
million ($2^{20}$) elements.
