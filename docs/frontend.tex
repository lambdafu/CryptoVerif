...
when the key is the first argument of the considered function or
$\texttt{"key\_last"}$ when it is the last argument.
...
\item When $\mathit{specialname}$ is
  not \texttt{rom} nor \texttt{rom\_partial}, an identifier
  $\mathit{p}$ such that $\mathit{p}(t, N, l)$ is the probability that
  an adversary breaks the PRF (resp. PRP) assumption in time $t$, with
  at most $N$ queries to the function $f$, of length at most $l$. The
  identifier $\mathit{p}$ must be declared with $\texttt{proba}\
  p$. This argument is omitted for random oracles because the
  probability is always 0.

\item A tuple of identifiers $(k, r, x, y, z, u)$, which are used to
...
$\texttt{if }args_1 = args_2 \texttt{ then }N_1\texttt{ else }N_2$, in the generated cryptographic transformation.
  ...
  The identifiers $\mathit{Ocoll}$ are used to form the oracle names in the generated equivalence (see below); they must not contan \texttt{\_}, and must be different from \texttt{O} and pairwise distinct.
