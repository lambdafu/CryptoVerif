...
{\tt defined}, {\tt diff}, {\tt do}, {\tt else}, {\tt eps\_find}, 
...
&\nonterm{vartypeb} ::= \neseq{ident} \texttt{:}\nonterm{ident}\\
&\phantom{\nonterm{vartypeb} =}\mid \neseq{ident} \texttt{ <= }\nonterm{ident}\\
&\nonterm{ident\_underscore} ::= \nonterm{ident}\\
&\phantom{\nonterm{ident\_underscore} =}\mid \texttt{\_}\\
&\nonterm{basicpat} ::= \nonterm{ident\_underscore}\\
&\phantom{\nonterm{basicpat} =}\mid \nonterm{ident\_underscore}\texttt{:}\nonterm{ident}\\
&\phantom{\nonterm{basicpat} =}\mid \nonterm{ident\_underscore}\texttt{ <= }\nonterm{ident}
\end{align*}
\end{minipage}
\begin{minipage}{7cm}
...
&\phqt \texttt{let } \nonterm{basicpat} \texttt{ = }\nonterm{letterm}\texttt{ in }\nonterm{queryterm}\\
&\phqt \nonterm{letterm}\\
&\nonterm{query} ::= \texttt{secret }\nonterm{ident}\ [\texttt{public\_vars}\ \seq{ident}]\ [\texttt{[}\neseq{ident}\texttt{]}]\\
...
\texttt{let}, \texttt{new}, \texttt{insert}, \texttt{get}, and \texttt{diff} which are similar to the corresponding
...
\item $\texttt{\_}[\texttt{:}T]$ underscore, possibly with a type. Matches any bitstring
(in type $T$), and ignores its value.
...
\texttt{\_} is allowed instead of $x$, and in this case the value of $M$
is simply ignored.
...
\item $\texttt{diff[}M_1,M_2\texttt{]}$ allows the user to define two processes: in the first process, the first argument of
\texttt{diff} is always used, while in the second process, the second argument of \texttt{diff} is always used. CryptoVerif shows indistinguishability between the obtained two processes.
...
\ifchannels
\item Let us now explain the output $\texttt{out(}c\texttt{[}M_1,
...
\item $\texttt{diff[}P_1,P_2\texttt{]}$ allows the user to define two processes: in the first process, the first argument of
\texttt{diff} is always used, while in the second process, the second argument of \texttt{diff} is always used. CryptoVerif shows indistinguishability between the obtained two processes. Internally, this is encoded by choosing a random bit \texttt{diff\_bit}, running the first argument of \texttt{diff} when \texttt{diff\_bit} is true and the second one when when \texttt{diff\_bit} is false, and showing the secrecy of \texttt{diff\_bit}. The variables that are public due to other queries (variables included in \texttt{public\_vars} in queries as well as variables on which there is a \texttt{query secret}) are considered public when proving the secrecy of \texttt{diff\_bit}.
...
\item \texttt{set guessRemoveUnique = false.}\\
\texttt{set guessRemoveUnique = true.}

When we use a guess transformation (\texttt{guess} or \texttt{guess\_branch})
and a (one-session) secrecy query is present, we must reprove that all \texttt{find[unique]}
in the game are really unique.
When \texttt{guessRemoveUnique = false} (the default), we reprove them.
When \texttt{guessRemoveUnique = true}, we instead remove the \texttt{[unique]}
annotations to avoid having to reprove them.
...
are renamed accordingly, by the transformation {\tt all\_simplify}
and by the simplification after the {\tt crypto} and {\tt SArename}
transformations.
...
When \texttt{simplifyAfterSARename = true}, apply simplification
(\texttt{remove\string_assign useless} if \texttt{autoRemoveAssignFindCond = false}, 
\texttt{remove\string_assign findcond} if \texttt{autoRemoveAssignFindCond = true},
\texttt{SArename new} if \texttt{autoSARename = true}, \texttt{simplify})
after each execution of the SArename transformation. This slows down
...
\item $\texttt{set maxElsefind = }n{\tt .}$ (default \texttt{50})

Maximum of facts guaranteed in \texttt{else} branches of \texttt{find} collected from a single term.
...
\item $\texttt{set maxGuess = }\nonterm{s}{\tt .}$ (default \texttt{size40})

  Sets the maximum size for which we can guess the value of a certain type.
  The argument $\nonterm{s}$ can be \texttt{default} or \texttt{passive}
  (size $2^{30}$), \texttt{small} (size $2^2$), or \texttt{size$n$} (size $2^n$).
...
\item \texttt{[autoSwapIf]} tells CryptoVerif to rewrite terms of the form $f(\dots,\texttt{if\_fun}(M_1,M_2,M_3),\dots)$ into $\texttt{if\_fun}(M_1,f(\dots,M_2,\dots),f(\dots,M_3,\dots))$, where \texttt{if\_fun} is a predefined test function that satisfies $\texttt{if\_fun(true,$x$,$y$)} = x$ and $\texttt{if\_fun(false,$x$,$y$)} = y$.
...
would like to prove. 
CryptoVerif allows two variants of the syntax for queries:
\begin{itemize}

\item In the recommended syntax, 
the part $[\seq{vartypeb}\texttt{;}]$ is omitted, and the query declaration is of the form
$\texttt{query }Q_1\texttt{;} \dots\texttt{;}Q_n$. The variables in correspondence
queries are explicitly quantified inside each query $Q_i$, as explained below.
\item
In the other syntax, the query declaration is of the form $\texttt{query }x_1\texttt{:}T_1\texttt{,} \ldots\texttt{,} x_n\texttt{:}T_n\texttt{;} Q_1\texttt{;} \dots\texttt{;}Q_n$. First, we declare the types of all variables $x_1, \ldots, x_n$
that occur in correspondence queries that follow. (We use $x_i \texttt{<=} N_i$ instead of
  $x_i\texttt{:} T_i$ when $x_i$ is of type $[1,N_i]$, where $N_i$ is
  a parameter, declared by $\texttt{param }N_i$.) Second, we give the queries $Q_1, \dots, Q_n$ themselves,
without any $\texttt{forall }x_1\texttt{:}T_1\texttt{,} \dots\texttt{,} x_j\texttt{:}T_j\texttt{;}$ and 
$\texttt{exists }y_1\texttt{:}T'_1\texttt{,} \dots\texttt{,} y_k\texttt{:}T'_k\texttt{;}$.
This form is not recommended because the quantifiers applied to the variables are less
obvious to the user: variables that occur before \texttt{==>} are universally quantified
and variables that occur after \texttt{==>} but not before \texttt{==>} are existentially quantified
just after \texttt{==>}.
\end{itemize}
The available queries $Q_i$ are as follows:
\begin{itemize}

\item ${\tt secret}\ x\ [\texttt{public\_vars}\ l]$ or\\
${\tt secret}\ x\ [\texttt{public\_vars}\ l]\ \texttt{[real\_or\_random]}$: show that the array $x$ is indistinguishable
...
\item ${\tt secret}\ x\ [\texttt{public\_vars}\ l]\ \texttt{[onesession]}$ or\\
${\tt secret}\ x\ [\texttt{public\_vars}\ l]\ \texttt{[real\_or\_random,onesession]}$: 
...
\item ${\tt secret}\ x\ [\texttt{public\_vars}\ l]\ \texttt{[reachability]}$:
shows that the adversary cannot compute any element of the array $x$,
even when it has access to the other elements of $x$ and to the variables
in $l$.
This query is allowed only when $x$ is of a \texttt{large} type
(or type declared with $\texttt{pcoll}n$ for $n \geq n'$ with $\texttt{set minAutoCollElim = pest}n'$),
that is, collisions can be eliminated between random values of that type.
Otherwise, the adversary would have a non-negligible probability
of finding the value of $x$ just by guessing.

\item ${\tt secret}\ x\ [\texttt{public\_vars}\ l]\ \texttt{[reachability,onesession]}$:
shows that the adversary cannot compute any element of the array $x$,
even when it has access to the variables in $l$.
This query is allowed only when $x$ is of a \texttt{large} type
(or type declared with $\texttt{pcoll}n$ for $n \geq n'$ with $\texttt{set minAutoCollElim = pest}n'$),
that is, collisions can be eliminated between random values of that type.
Otherwise, the adversary would have a non-negligible probability
...
This notion of secrecy as ``the adversary cannot compute'' is less common
in the computational model than ``the adversary cannot distinguish from
random'', but it is still used (e.g. in the property of one-wayness
or in the computational Diffie-Hellman assumption).

\item ${\tt secret}\ x\ [\texttt{public\_vars}\ l]\ \texttt{[cv\_bit]}$:
shows that the adversary cannot guess the value of the boolean variable $x$
significantly better than at random.
It applies only when $x$ is a boolean variable defined under no replication.
When the process chooses $x$ randomly and executes different code
depending on whether $x$ is true or false, this property also
shows that the process with $x$ true is indistinguishable from the
process with $x$ false.

The options \texttt{cv\_onesession}, \texttt{cv\_real\_or\_random},
\texttt{cv\_reachability} are also allowed, and synonym of the similar
options without \texttt{cv\_} prefix. The only difference is that
options that start with \texttt{cv\_} apply to CryptoVerif only,
while options that start neither with \texttt{cv\_} nor with \texttt{pv\_}
apply to both CryptoVerif and ProVerif.
All options starting with \texttt{pv\_} are also allowed,
but ignored: they are for ProVerif.

\item $[\texttt{forall }x_1\texttt{:}T_1\texttt{,} \dots\texttt{,} x_j\texttt{:}T_j\texttt{;}]\ M_0\ [\texttt{public\_vars}\ l]$, where in $M_0$, \texttt{==>} is not allowed under \texttt{\&\&}, \texttt{||}, or \texttt{exists}, and \texttt{||} and \texttt{exists} are allowed only after \texttt{==>}, so that after replacing variables bound by assignments by their value and moving existential quantifiers just after \texttt{==>}, $M_0$ is of form $M \texttt{ ==> } [\texttt{exists }y_1\texttt{:}T'_1\texttt{,} \dots\texttt{,} y_k\texttt{:}T'_k\texttt{;}]\ M'$ or $M$, where $M$ and $M'$ do not contain \texttt{==>}. The query $M$ is an abbreviation for $M \texttt{ ==> false}$, so we only need to consider $M \texttt{ ==> } [\texttt{exists }y_1\texttt{:}T'_1\texttt{,} \dots\texttt{,} y_k\texttt{:}T'_k\texttt{;}]\ M'$. (We use $x_i \texttt{<=} N_i$ instead of
  $x_i\texttt{:} T_i$ when $x_i$ is of type $[1,N_i]$, where $N_i$ is
  a parameter, declared by $\texttt{param }N_i$. We use the same notation for $y_i$.)

The variables $x_1, \dots, x_j$ are the variables that occur in $M$,
and the variables $y_1, \dots, y_k$ are the variables of $M'$ that do not
occur in $M$. (In particular, a universally quantified variable $x_i$ is 
not allowed to occur in $M'$ and not in $M$.)
CryptoVerif shows that, for all values of $x_1, \dots, x_j$,
if $M$ is true then there exist values of $y_1, \dots, y_k$ such that $M'$ is true, 
even when the variables in $l$ are public.
...
\item The function {\tt if\_fun} takes three arguments, one of type {\tt bool} and two arguments of the same type. It satisfies $\texttt{if\_fun(true,$x$,$y$)} = x$ and $\texttt{if\_fun(false,$x$,$y$)} = y$.
