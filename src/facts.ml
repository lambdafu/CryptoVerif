...
let rec orient t1 t2 = 
  match t1.t_desc, t2.t_desc with
    ...
    (not (Terms.is_restr b1 && Terms.get_pcoll1 b1.btype High <= !Settings.tysize_MIN_Coll_Elim)) &&
    (not (Terms.is_restr b2 && Terms.get_pcoll1 b2.btype High <= !Settings.tysize_MIN_Coll_Elim)) ->
      (* Both orientations would be possible, try to discriminate using
         priorities *)
      b1.priority >= b2.priority
  | (Var(b,l), _) when
    (not (Terms.refers_to b t2)) && 
    (not (Terms.is_restr b && Terms.get_pcoll1 b.btype High <= !Settings.tysize_MIN_Coll_Elim)) -> true
