...
let simplify_equal t1 t2 =
  ...
  | FunApp(f1,[]), FunApp(f2,[]) when
	...
	(* Apply the "diff_constants" setting only to constants of
	   a bitstring type, so that it is not applied to constant
           replication indices [i_tested] generated by the "guess" 
           transformation *)
	((snd f1.f_type).tcat = BitString) ->
      raise Contradiction
  | _ -> raise Unchanged
