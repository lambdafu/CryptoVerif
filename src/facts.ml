...
and simplif_add depth dep_info simp_facts fact =
  if !Settings.debug_simplif_add_facts then
    begin
      ...
      print_string " knowing\n"; Display.display_facts simp_facts; print_newline();
    end;
...
let get_initial_history_args (pp, args) =
...
let get_initial_history pp =
  ...
  | Some (cur_array,_,_,_,_,_,n) -> 
      get_initial_history_args (pp, List.map Terms.term_from_repl_index cur_array)
...
let get_compatible_def_nodes history pps b l =
  List.filter (fun n ->
    let pp = (n.definition_success,l) in
    (match history with
      None -> true
    | Some h -> Incompatible.is_compatible_history pp h) &&
    (List.for_all (Incompatible.both_pp_ppl pp) pps)
      ) b.def 
...
(* [add_facts] collects the facts that are known to hold when [br = b[l]]
   is defined:
   - The facts are collected in [fact_accu_opt], when it is not [None].
   - The program points are collected in [seen_pps]
   - The defined variables are collected in [seen_refs]
   Summary of the algorithm (simplified):
...
[add_facts] must not be used to remove elements
...
according to [add_facts] would remove too many elements, and
break the code of SArename.
Use [reduced_def_list] above to remove useless elements of def_list.
   *)

let rec add_facts history fact_accu_opt seen_pps seen_refs done_refs ((b,l) as br) =
  (* print_string "Is defined "; Display.display_var b l; print_newline(); *)
  (* Invariant: [br] is never in [!done_refs], that is checked before calling this function *)
  if List.for_all (check_non_nested [] [b]) l then
    begin
      done_refs := br :: (!done_refs);
      let nodes_b_def = get_compatible_def_nodes history (!seen_pps) b l in
      ...
      (* add program point *)
      let pp = (List.map (fun n -> n.definition_success) nodes_b_def, l) in
      seen_pps := List.filter (fun pp' -> not (Incompatible.implies_ppl pp pp')) (!seen_pps);
      seen_pps := List.map (fun (ppl',args') ->
	let ppl'' = List.filter (fun pp' -> Incompatible.both_pp_ppl (pp',args') pp) ppl' in
	(* if List.length ppl'' < List.length ppl' then
	  begin
	    print_string "Modified ";
	    Display.display_ppl (ppl', args');
	    print_string " into ";
	    Display.display_ppl (ppl'', args');
	    print_string " because of addition of ";
	    Display.display_ppl pp;
	    print_newline()
	  end;
           TODO If a program point is modified, I could redo the addition
	   of facts and binderrefs for this program point.
	   With which history? Should I keep the history associated with
	   each program point? For simplicity, I do not do that. *)
	(ppl'', args')
			   ) (!seen_pps);
      if not (List.exists (fun pp' -> Incompatible.implies_ppl pp' pp) (!seen_pps)) then
	seen_pps := pp :: (!seen_pps);
      (* add def vars *)
	...
	in
	if not (Terms.mem_binderref new_br (!done_refs)) then
	  begin
	    Terms.add_binderref new_br seen_refs;
	    done_refs := new_br :: (!done_refs)
	  end
	  ) def_vars_above_def;
      ...
      (* add facts *)
      begin
      match fact_accu_opt with
      | None -> ()
      | Some fact_accu ->
	  begin
            (* add facts from compatibility with [history] and [seen_pps] *)
	    begin
	      match history with
		None -> ()
	      | Some h ->
		  fact_accu := Incompatible.facts_compatible_history (!fact_accu) pp h
	    end;
	    List.iter (fun pp' ->
	      fact_accu := Incompatible.both_ppl_ppl_add_fact (!fact_accu) pp' pp
		   ) (!seen_pps);
	    (* add facts from defined vars *)
      ...
      (* recursive call *)
      add_facts_list history' fact_accu_opt seen_pps seen_refs done_refs def_vars_at_def'
    end

and add_facts_list history fact_accu_opt seen_pps seen_refs done_refs br_list =
  let br_list = List.filter (fun br -> not (Terms.mem_binderref br (!done_refs))) br_list in
  List.iter (fun br -> Terms.add_binderref br seen_refs) br_list;
...
let facts_from_defined known_pps known_def_vars history def_list =
  ...
  let seen_refs = ref [] in
  let seen_pps = ref known_pps in 
  add_facts_list history (Some fact_accu) seen_pps seen_refs (ref known_def_vars) (!def_list_subterms);
  (!fact_accu, !seen_pps, !seen_refs)
...
let get_def_vars_at pp  = 
  match Incompatible.get_facts pp with
  | None -> ([], [], None)
  | Some (cur_array,_,_,def_vars,_,_,n) ->
      let args = List.map Terms.term_from_repl_index cur_array in
      let done_refs = ref (get_def_vars_above n) in
      let seen_refs = ref (!done_refs) in
      (* Note: def_vars contains n.def_vars_at_def *)
      let history = get_initial_history_args (pp, args) in
      let seen_pps = ref [([pp],args)] in
      add_facts_list history None seen_pps seen_refs done_refs def_vars;
      (!seen_pps, !seen_refs, history)
...
let get_facts_at pp =
  match Incompatible.get_facts pp with
    None -> ([], [], [], None)
  | Some(cur_array, true_facts, _, def_vars, _,_, n) ->
      let args = List.map Terms.term_from_repl_index cur_array in
      let done_refs = ref (get_def_vars_above n) in
      let seen_refs = ref (!done_refs) in
      ...
      let history = get_initial_history_args (pp,args) in
      let seen_pps = ref [([pp],args)] in
      add_facts_list history (Some fact_accu) seen_pps seen_refs done_refs def_vars;
      (!fact_accu, !seen_pps, !seen_refs, history)

let get_facts_at_args known_pps known_def_vars (pp,args) =
  match Incompatible.get_facts pp with
    None -> ([], [], [])
  | Some(cur_array, true_facts, _, def_vars, _,_, n) ->
      List.iter2 (fun ri t -> ri.ri_link <- TLink t) cur_array args;
      let def_vars_args = Terms.copy_def_list Terms.Links_RI def_vars in
      let vars_above_args = Terms.copy_def_list Terms.Links_RI (get_def_vars_above n) in
      let done_refs = ref (Terms.union_binderref vars_above_args known_def_vars) in
      let seen_refs = ref (!done_refs) in
      let fact_accu = ref (List.map (Terms.copy_term Terms.Links_RI) (filter_ifletfindres true_facts)) in
      (* remove the links *)
      List.iter (fun ri -> ri.ri_link <- NoLink) cur_array;
      (* Note: def_vars contains n.def_vars_at_def *)
      let history = get_initial_history_args (pp,args) in
      List.iter (fun pp' ->
	fact_accu := Incompatible.both_pp_ppl_add_fact (!fact_accu) (pp,args) pp'
	     ) known_pps;
      let seen_pps = ref (([pp],args) :: known_pps) in
      add_facts_list history (Some fact_accu) seen_pps seen_refs done_refs def_vars_args;
      (!fact_accu, !seen_pps, !seen_refs)
...
let rec filter_def_list known_pps known_vars history accu = function
    [] -> accu
  | (br::l) ->
      let (_,implied_br) = def_vars_from_defined known_pps known_vars history [br] in
      let accu' = Terms.setminus_binderref accu implied_br in
      let l' = Terms.setminus_binderref l implied_br in
      filter_def_list known_pps known_vars history (br::accu') l'
...
let get_def (already_pps, already_defined, history_pp) newly_defined_opt def_list =
  match newly_defined_opt with
  | Some newly_defined -> newly_defined
  | None -> 
      let (_, newly_defined) = def_vars_from_defined already_pps already_defined history_pp def_list in
      newly_defined
	
let update_def_list_term (already_pps, already_defined, history_pp) newly_defined_opt bl def_list tc' p' =
  let newly_defined = get_def (already_pps, already_defined, history_pp) newly_defined_opt def_list in
  ...
  let needed_occur = Terms.setminus_binderref (!accu_needed_subterm) already_defined in
  ...
  let (_, implied_needed_occur) =
    try
      def_vars_from_defined already_pps already_defined history_pp needed_occur
    with Contradiction ->
      (* In general, the variables [needed_occur] should be defined,
	 or when they are not defined, the branch of find could be
	 removed. However, there is an exception to this situation:
	 in [Transf_expand], when a variable originally defined
	 in the branch of a find is moved outside: it cannot be defined
	 at the program point of the find in the initial game. *)
      ([], needed_occur)
  in
  (* We compute the subterms of [def_list]. Considering the subterms as well
     avoids keeping a term when some of its subterms are enough.
     It is important that the subterms appear before the term in [!def_list_subterm],
     which is the case given how [Terms.close_def_subterm] is implemented. *)
  let def_list_subterm = ref [] in
  List.iter (Terms.close_def_subterm def_list_subterm) (List.rev def_list);
  let def_list'' = Terms.setminus_binderref 
      (Terms.setminus_binderref (!def_list_subterm) implied_needed_occur) already_defined in
  let def_list3 = remove_subterms [] (needed_occur @ (filter_def_list already_pps already_defined history_pp [] def_list'')) in
  let def_list4 = 
...
let update_def_list_process (already_pps, already_defined, history_pp) newly_defined_opt bl def_list t' p1' =
  let newly_defined = get_def (already_pps, already_defined, history_pp) newly_defined_opt def_list in
  (* Compute in [accu_needed] the variable references that need to be included in the "defined"
  ...
  if not (List.for_all (fun br -> Terms.mem_binderref br newly_defined) needed_occur) then
    begin
      ...
      Display.display_def_list needed_occur;
      print_newline();
      print_string "Newly defined = ";
      Display.display_def_list newly_defined;
      print_newline();
    end; *)
  (* In case the definition of all needed variables 
     cannot be inferred from the original defined condition,
...
(* [add_facts_cases] below is a modified version of
   add_facts to distinguish cases depending on the
...
let rec add_facts_cases history (fact_accu, fact_accu_cases) seen_pps seen_refs done_refs ((b,l) as br) =
  ...
  if (List.for_all (check_non_nested [] [b]) l) &&
	...
	(ppl'', args')
      ...
      let def_vars_above_def = Terms.intersect_list (==) (List.map (get_def_vars_above2 history') nodes_b_def) in
      (* put links for the substitution b.args_at_creation -> l
         We must not raise an exception while the links are sets 
	 (because in this case, they will incorrectly remain). *)
      let bindex = b.args_at_creation in
      List.iter2 (fun b t -> b.ri_link <- TLink t) bindex l;
      List.iter (fun b ->
	let new_br = (b, List.map (fun ri -> match ri.ri_link with
	...
	in
	Terms.add_binderref new_br seen_refs
        (* Do not add [new_br] to [done_refs], because in case [new_br]
	   belongs to the binders defined in the future at [b[l]],
	   adding [new_br] explicitly may lead to distinguish cases
	   Manifests in examples-1.28-converted/kerberos/BasicKerberos_AS.cv
	...
	List.map2 (fun fact_accu n ->
	  let fact_accu' = 
	    match history with
	      None -> fact_accu
	    | Some h ->
		Incompatible.facts_compatible_history fact_accu ([n.definition_success], l) h
	  in
	  List.fold_left (fun fact_accu pp' ->
	    Incompatible.both_pp_ppl_add_fact fact_accu (n.definition_success, l) pp') fact_accu' (!seen_pps)
	...
	intersect_list_cases (!seen_refs) Terms.equal_binderref def_vars_at_def_list'
      ...
      (* recursive call *)
      List.iter (add_facts_cases history' (fact_accu, fact_accu_cases) seen_pps seen_refs done_refs) def_vars_at_def_common;
	...
	fold_left3 (fun accu history_1case true_facts_at_def_1case missed_br ->
	  try 
	    ...
	    let seen_pps = ref (!seen_pps) in 
	    add_facts_list history_1case (Some missed_facts) seen_pps seen_refs_tmp done_refs_tmp missed_br;
      ...
      in
      if not (List.exists (fun fl -> fl == []) true_facts_at_def_cases) then
	fact_accu_cases := true_facts_at_def_cases :: (!fact_accu_cases);
    end

(* [get_facts_full_block_args known_pps known_def_vars pp] returns the facts
   that are known to hold at the end of the input...output block containing
   program point [pp].
   Returns the program points known to be executed and the defined variables
   at [pp], not at the end of the block.
   (Future defined variables are handled separately.)
   When [Settings.corresp_cases] is true, distinguishes cases to improve
   precision. *)

let get_facts_full_block_args known_pps known_def_vars (pp,args) =
  match Incompatible.get_facts pp with
    None -> ([], [], [], [])
  | Some(cur_array, true_facts, _, def_vars, fut_true_facts,_, n) ->
      List.iter2 (fun ri t -> ri.ri_link <- TLink t) cur_array args;
      let def_vars_args = Terms.copy_def_list Terms.Links_RI def_vars in
      let vars_above_args = Terms.copy_def_list Terms.Links_RI (get_def_vars_above n) in
      let done_refs = ref (Terms.union_binderref vars_above_args known_def_vars) in
      let seen_refs = ref (!done_refs) in
      let fact_accu = ref (List.map (Terms.copy_term Terms.Links_RI) (filter_ifletfindres (fut_true_facts @ true_facts))) in
	...
	begin
	  seen_refs := Terms.union_binderref def_vars_args (!seen_refs);
	  List.iter (add_facts_cases history (fact_accu, fact_accu_cases) seen_pps seen_refs done_refs) def_vars_args
	end
      else
	begin
	  add_facts_list history (Some fact_accu) seen_pps seen_refs done_refs def_vars_args
	end;
      (!fact_accu, !fact_accu_cases, !seen_pps, !seen_refs)
