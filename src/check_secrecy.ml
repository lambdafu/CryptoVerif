...
type state =
    { cur_array : repl_index list;
      ...
      b : binder;
      lidx : term list;
      facts : repl_index list * simp_facts * binderref list * (term list * program_point) list }
...
let rec check_usage_term state used t =
  match t.t_desc with
	...
	begin
	  ...
	  try
	    let eq_index = List.map2 Terms.make_equal state.lidx l in 
	    let (lidx', (all_indices, simp_facts, defined_refs, _)) = add_facts_at state.facts state.cur_array eq_index (DTerm t) in
	    let facts2 = 
...
let check_distinct collector b =
  ...
  let distinct_def
	...
	let facts2 = 
	  if !Settings.elsefind_facts_in_success then
	    Facts_of_elsefind.get_facts_of_elsefind_facts (!whole_game) (r_index1 @ r_index2) simp_facts1 
	      def_vars
	  else
	    []
	in
	...
	Terms.add_to_collector collector (CollectorFacts(r_index1 @ r_index2, [(index1, d1.definition); (index2, d2.definition)], simp_facts2, def_vars));
	false
...
let check_secrecy g collector b pub_vars one_session =
  ...
  if one_session then
	...
	begin
	  ...
	  let p = Depanal.final_add_proba() in 
	  let proba = p @ p in
