...
(* Generate the equivalence for random functions *)
...
let make_random_fun_equiv f key_first ((id_k, ext_k), id_r, id_x, id_y, id_z, id_u) proba_fun (oracles : (string * random_fun_oracle_t) list) collision_matrix =
  let var_num_state = Terms.get_var_num_state() in
  let rev_opt l = if key_first then l else List.rev l in
  let (tyallargs, tyres) = f.f_type in
  let (tykey,tyargs) =
    match rev_opt tyallargs with
    | [] -> Parsing_helper.internal_error "For special equivalence random_fun, the function should have at least the key plus one other argument"
    | tykey::tyargs -> tykey, tyargs
  in
  let k =
  ...
  in
  let callf args =
    Terms.app f (rev_opt (List.map Terms.term_from_binder (k::args)))
  in
  ...
  let prev_res (o_name', o_ty', idx', b_idx', param', args', res', _, _, _) =
    ...
    | None ->
	Terms.app f (rev_opt ((Terms.term_from_binder k)::
  ...
  Display.fun_out print (fun () ->
    ...
    List.iter (fun ((o_name, o_ty, idx, b_idx, param, args, res, _, _, _) as oracle) ->
      match o_ty with
      | Basic Normal ->
	  normal_calls := (Count param) :: (!normal_calls);
    ...
    let all_maxlength = List.map (fun accu -> Polynom.p_max (!accu)) all_maxlength_accu in
    let normal_calls = Polynom.p_sum (!normal_calls) in
    let all_calls = Polynom.p_sum (!all_calls) in
    let proba_coll = Polynom.p_sum (!proba_coll) in
    let total_proba = Polynom.p_add (proba_fun normal_calls all_calls all_maxlength, proba_coll) in
...
let random_fun f_type is_partial (equiv_name, gen_ext) args_at_equiv call =
  ...
  let expected_args() =
    ...
    "(k, r, x, y, z, u), collisions)\nwhere
- key_pos is either \"key_first\" or \"key_last\" depending on the position of the key argument in function f
- f is a function\n"^
    (if f_type == ROM then "" else "- proba(t, N, l) is the probability that an adversary breaks the "^(if f_type == PRP then "PRP" else "PRF")^" assumption
  in time t, with at most N queries to the function, of length at most l\n")^
...
- y and z are the identifiers used for arguments of the two calls to f in oracles generated by the collision \"Ocoll: new r1:T; new r2:T; forall a1:T1...an:Tn; t\"
- u is the identifier used for find indices 
- collisions describes collisions and must be a tuple of strings
The arguments (k, r, x, y, z, u) and collisions may be omitted."
  in
  let key_first, r_args_at_equiv =
    match args_at_equiv with
    | (SpecialArgString ("key_first", _), _)::rest ->
	true, rest
    | (SpecialArgString ("key_last", _), _)::rest ->
	false, rest
    | (_, ext)::rest ->
  ...
  begin
    let pos_key, pos_arg_prp = if key_first then "first", "second" else "last", "first" in
    if f_type == PRP then
      match rev_opt ty_all_args with
      | [ ty_key; ty_arg ] when ty_arg == ty_res ->
	  if ty_key.toptions land Settings.tyopt_CHOOSABLE == 0 then
	    raise (Error("Special equivalence "^equiv_name^": one must be able to choose randomly a bitstring from the key type "^ty_key.tname^" ("^pos_key^" argument of "^f.f_name^")", ext_f));
      | _ ->
	  raise (Error("Special equivalence "^equiv_name^": the function "^f.f_name^" must have two arguments, the "^pos_key^" argument is a key, and its "^pos_arg_prp^" argument must have the same type as its result (domain of the permutation)", ext_f))
    else
      match rev_opt ty_all_args with
      | ty_key :: _ :: _ ->
	  if ty_key.toptions land Settings.tyopt_CHOOSABLE == 0 then
	    raise (Error("Special equivalence "^equiv_name^": one must be able to choose randomly a bitstring from the key type "^ty_key.tname^" ("^pos_key^" argument of "^f.f_name^")", ext_f));
      | _ ->
	  raise (Error("Special equivalence "^equiv_name^": the function "^f.f_name^" must have at least two arguments (the "^pos_key^" argument is a key)", ext_f))
  end;
  let proba_fun, r_args_at_equiv =
    match r_args_at_equiv, f_type with
    | _, ROM ->
	(fun _ _ _ -> Zero),  r_args_at_equiv
    | (SpecialArgId (proba_f, ext), _)::rest, (PRF | PRP) ->
	begin
	  try
	    match Stringmap.StringMap.find proba_f (!Stringmap.env) with
	    | EProba p ->
		(fun normal_calls all_calls all_maxlength ->
		  let proba_f = Proba(p, AttTime :: all_calls :: all_maxlength) in
		  if f_type == PRF then
		    proba_f
		  else
		    Add(proba_f, Mul(normal_calls, Mul(Sub(normal_calls, Cst 1.0), Proba.pcoll2rand ty_res)))
		    ), rest
	    | _ ->
  ...
  let ids, r_args_at_equiv =
    match r_args_at_equiv with
    | (SpecialArgTuple [ SpecialArgId id_k, _; SpecialArgId id_r, _; SpecialArgId id_x, _; SpecialArgId id_y, _; SpecialArgId id_z, _; SpecialArgId id_u, _ ], _) :: rest ->
  ...
  Terms.set_var_num_state var_num_state;
  make_random_fun_equiv f key_first ids proba_fun oracles checked_collision_matrix
