...
val def_vars_and_facts_from_term : game option -> bool -> bool -> term -> term list * binderref list * elsefind_fact list

(* [else_info_from_find_branch g_opt expanded br] returns 
   information certainly true when the condition in the branch [br] fails
   (elsefind facts) *) 
val else_info_from_find_branch : game option -> bool -> 'a findbranch -> elsefind_fact list
    
(* [add_else_info_find g_opt expanded l0] adds information collected 
   by [else_info_from_find_branch] to each branch of find in [l0] *)
val add_else_info_find : game option -> bool -> 'a findbranch list -> ('a findbranch * elsefind_fact list) list
...
(* [info_from_find_branch g_opt expanded br] returns a pair of
   ...
   (elsefind facts) *) 
val info_from_find_branch : game option -> bool -> 'a findbranch -> (term list * binderref list * elsefind_fact list) * elsefind_fact list

(* [add_info_find g_opt expanded l0] adds information collected 
   by [info_from_find_branch] to each branch of find in [l0] *)
val add_info_find : game option -> bool -> 'a findbranch list -> ('a findbranch * ((term list * binderref list * elsefind_fact list) * elsefind_fact list)) list
