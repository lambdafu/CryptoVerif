...
%start all
%type <Ptree.pall * Ptree.process_e> all
...
libelem:
	...
	{ [FunDecl($2, $4, $7, $8), parse_extent()] }
|       EVENT IDENT DOT 
        { [EventDecl($2, []), parse_extent()] }
|       EVENT IDENT LPAREN identlist RPAREN DOT
        { [EventDecl($2, $4), parse_extent()] }
|	FORALL vartypelist SEMI term DOT 
	{ [Statement($2, $4), parse_extent()] }
|       EQUATION IDENT LPAREN identlist RPAREN DOT
        { [BuiltinEquation($2, $4), parse_extent()] }
|	LET IDENT EQUAL process DOT
	{ [PDef($2,$4), parse_extent()] }
|       SET IDENT EQUAL IDENT DOT
        { [Setting($2,S $4), parse_extent()] }
|       SET IDENT EQUAL INT DOT
        { [Setting($2,I $4), parse_extent()] }
|       QUERY queryseq DOT 
        { [Query($2), parse_extent()] }
|       PARAM neidentlist options DOT
        { List.map (fun x -> (ParamDecl(x, $3), parse_extent())) $2 }
|       PROBA IDENT DOT 
        { [ProbabilityDecl($2), parse_extent()] }
|       CONST neidentlist COLON IDENT DOT 
        { List.map (fun x -> (ConstDecl(x,$4), parse_extent())) $2 }
|       CHANNEL neidentlist DOT 
        { List.map (fun x -> (ChannelDecl(x), parse_extent())) $2 }
|       TYPE IDENT options DOT
        { [TypeDecl($2,$3), parse_extent()] }
|       EQUIV eqname eqmember EQUIVLEFT probaf EQUIVRIGHT optpriority eqmember DOT
        { [EqStatement($2, $3, $8, $5, $7), parse_extent()] }
|       COLLISION newlist FORALL vartypelist SEMI term EQUIVLEFT probaf EQUIVRIGHT term DOT
        { [Collision($2, $4, $6, $8, $10), parse_extent()] }
|       COLLISION newlist term EQUIVLEFT probaf EQUIVRIGHT term DOT
        { [Collision($2, [], $3, $5, $7), parse_extent()] }
|       DEFINE IDENT LPAREN identlist RPAREN LBRACE lib RBRACE
        { [Define($2, $4, $7), parse_extent()] }
|       EXPAND IDENT LPAREN identlist RPAREN DOT
        { [Expand($2, $4), parse_extent()] }
|       PROOF LBRACE proof RBRACE 
        { [Proofinfo($3), parse_extent()] }
|       IMPLEMENTATION impllist DOT 
        { [Implementation($2), parse_extent()] }
|       TABLE IDENT LPAREN neidentlist RPAREN DOT
        { [TableDecl($2,$4), parse_extent()] }
|       LETFUN IDENT EQUAL term DOT
        { [LetFun($2,[],$4), parse_extent()] }
|       LETFUN IDENT LPAREN vartypelist RPAREN EQUAL term DOT
        { [LetFun($2,$4,$7), parse_extent()] }
...
|	REPL IDENT process %prec REPL
	{ PRepl (ref None,None,$2,$3), parse_extent() }
|	REPL IDENT LEQ IDENT process %prec REPL
	{ PRepl (ref None,Some $2,$4,$5), parse_extent() }
|	INT 
	{ let x = $1 in
