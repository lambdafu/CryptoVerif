...
let add_in_env1 s t cur_array =
  if Hashtbl.mem binder_env s then
    ...
    let b = (if !current_location = InProcess then Terms.create_binder else Terms.create_binder0) s t cur_array in
...
let rec check_term1 in_find_cond cur_array = function
  ...
  | PEventE(t,p), _ ->
      check_term1 in_find_cond cur_array t;
      check_term1 in_find_cond cur_array p
  | PGetE(tbl, patlist, topt, p1, p2), _ ->
	 ...
	 We must appropriately forbid array accesses to the variables they define. *)
      ...
      check_term1 in_find_cond cur_array p1;
      check_term1 in_find_cond cur_array p2
  | PInsertE(tbl,tlist,p),_ ->
      List.iter (check_term1 in_find_cond cur_array) tlist;
      check_term1 in_find_cond cur_array p      
...
let rec check_fungroup1 all_ch cur_array = function
    PReplRestr((repl_index_ref, idopt, (rep,ext)), restrlist, funlist) ->
      ...
      List.iter (check_fungroup1 all_ch cur_array') funlist
...
let rec check_no_iffindletnewevent ext t =
  match t.t_desc with
  | Var (_,l) | FunApp(_,l) ->
      List.iter (check_no_iffindletnewevent ext) l
  | ReplIndex _ -> ()
  | TestE _ | LetE _ | FindE _ | ResE _ | EventAbortE _ | EventE _ | GetE _ | InsertE _ ->
      Parsing_helper.input_error ((instruct_name t) ^ " at " ^ (in_file_position ext t.t_loc) ^
				  " should not occur in defined condition") ext
...
let rec check_term cur_array env = function
  ...
  | PEventE((PFunApp((s,ext0),tl), ext), p), ext2 ->
	  ...
	  match StringMap.find s env with
		  ...
		  Terms.new_term Settings.t_bitstring ext2
		    (FunApp(tupf, List.map Terms.term_from_repl_index cur_array))
	        in
	        let p' = check_term cur_array env p in
                let event =
  ...
  | PGetE((id,ext),patl,topt,p1,p2),ext2 -> 
      ...
      let p2' = check_term cur_array env p2 in
	...
	| Some t -> 
	    ...
	    Some t'
      in
      let p1' = check_term cur_array env' p1 in
...
and check_br cur_array env ((s,ext), tl) =
  let tl' = List.map (check_term cur_array env) tl in
  List.iter2 (fun t t' -> check_no_iffindletnewevent (snd t) t') tl tl';
  begin
    try
      match Hashtbl.find binder_env s with
	Uniq b -> 
...
let rec check_lm_fungroup2 cur_array cur_restr env all_ch seen_ch seen_repl = function
	...
	Parsing_helper.input_error "In an equivalence, different functions must have a different number of repetitions" ext;
      ...
      let funlist' = List.map (check_lm_fungroup2 cur_array' (restrlist'::cur_restr) env' all_ch seen_ch seen_repl) funlist in
      (* Remove useless new *)
  ...
  | PFun(chref, arglist, tres, (priority, options)) ->
      let (s, ext) = !chref in
      let s =
	if s = "@dummy_channel" then
      ...
      in
      chref := (s,ext);
      let ch' = check_channel_id (s,ext) in
...
let rec check_rm_fungroup2 options2 cur_array env fg0 fg = 
  ...
  | Fun(ch0, arglist0, tres0, priority0), PFun(chref, arglist, tres, _) ->
      let (ch, ext) = !chref in
      let ch =
...
let check_eqstatement (name, (mem1, ext1), (mem2, ext2), proba, (priority, options)) =
  ...
  List.iter (fun (fg, _, _) -> check_fungroup1 all_ch [] fg) mem1; (* Builds binder_env *)
  let count_exist = ref 0 in
  let mem1' = List.map (fun (fg, mode, ext) ->
    let res = (check_lm_fungroup2 [] [] (!env) (!all_ch) seen_ch seen_repl fg,
  ...
  Hashtbl.clear binder_env;
  List.iter (fun (fg, _, _) -> check_fungroup1 all_ch [] fg) mem2; (* Builds binder_env *)
  let mem2' = List.map2 (fun (fg0, _) (fg, mode, _) -> 
    check_rm_fungroup2 (!options2') [] (!env) fg0 fg, check_mode true mode) mem1' mem2 
  in
  let new_equiv = (name, mem1',mem2', (if proba' = Zero then [] else [SetProba proba' ]), !options', !options2') in
  equivalences := new_equiv :: (!equivalences);
...
let check_collision env (restr, forall, t1, proba, t2) =
  ...
  let new_collision = (restr', forall', t1', proba', t2') in
...
let rec rename_fungroup = function
  ...
  | PFun(i, larg, r, n) ->
      PFun(ref (rename_ie_dummy_channel (!i)),
...
let rename_query = function
  ...
  | PQEvent(decl,t1,t2) -> 
      PQEvent(List.map (fun (x,t) -> (rename_ie x, rename_ty t)) decl, 
...
let apply argl paraml already_def def =
  ...
  let def' = List.map (fun (decl, ext) -> (rename_decl decl, ext)) def in
  rename_table := StringMap.empty;
  def'
...
let decl_changes = function
  ...
  | FunDecl((s1,ext1),l,(sr,extr),f_options),ext0 ->
	  ...
	  if sopt = "decompos" then 
	    ...
	    Changes.add_change extopt (Replace "projection")
	  else if sopt = "uniform" then
	    ()
	  else if sopt = "compos" then
	    (* "compos" becomes "data" *)
	    Changes.add_change extopt (Replace "data")
	  else
  ...
  | Collision s,ext0 ->
      Changes.add_change ext0 (ChCollision s)
  | Query l,ext0 ->
      Changes.add_change ext0 (ChQuery l)
  | PDef((s1,ext1),p),ext0 ->
      Changes.add_change ext0 Remove
  


	
let rec check_one = function
    ParamDecl((s,ext), options), ext0 ->
      let size =
	match options with
  ...
  | ConstDecl((s1,ext1),(s2,ext2)),ext0 ->
  ...
  | BuiltinEquation(eq_categ, l_fun_symb),ext0 ->
  ...
  | Define((s1,ext1),argl,def),ext0 ->
	...
	let rec expand_inside_macro = function 
	  ...
	  | (Query _, ext0)::l ->
	      input_error "Sorry: queries cannot be converted inside macros" ext0
	  | a::l -> a::(expand_inside_macro l)
	  | [] -> []
	in
	List.iter decl_changes def;
...
let rec check_all (l,p) =
  ...
  let (p',_,_) = result in
  Changes.add_change (snd p) (ChProcess p');
  result
