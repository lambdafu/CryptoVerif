...
let rec pseudo_expand_term t = 
  match t.t_desc with
    Var(b,l) ->
      ...
      (fun context -> lex (fun li -> context (Terms.build_term t (Var(b,li)))))
  | ReplIndex _ -> (fun context -> context t)
  | FunApp(f,l) ->
      let lex = pseudo_expand_term_list l in
      (fun context -> lex (fun li -> context (Terms.build_term t (FunApp(f,li)))))
  | TestE(t1,t2,t3) ->
      let t1ex = pseudo_expand_term t1 in
      let t2ex = pseudo_expand_term t2 in
      let t3ex = pseudo_expand_term t3 in
      (fun context ->
	t1ex (fun t1i ->
          (* Some trivial simplifications *)
  ...
  | LetE(pat, t1, t2, topt) ->
      let patex = pseudo_expand_pat pat in
      let t1ex = pseudo_expand_term t1 in
      let t2ex = pseudo_expand_term t2 in
      begin
	match topt with
  ...
  | ResE(b, t) ->
      let tex = pseudo_expand_term t in
      (fun context -> let t' = tex context in Terms.build_term t' (ResE(b, t')))
  | EventAbortE _ ->
...
| (a::l) -> 
    ...
    let lex = pseudo_expand_term_list l in
    (fun context -> aex (fun a' -> lex (fun l' -> context (a'::l'))))

and pseudo_expand_pat = function
    PatVar b -> (fun context -> context (PatVar b))
  | PatTuple (ft,l) ->
      let lex = pseudo_expand_pat_list l in
      (fun context -> lex (fun li -> context (PatTuple (ft,li))))
  | PatEqual t ->
      let tex = pseudo_expand_term t in
      (fun context -> tex (fun ti -> context (PatEqual ti)))
...
let rec expand_term t = 
  ...
  | FunApp(f,l) ->
      ...
      (fun context -> lex (fun li -> context (Terms.build_term t (FunApp(f,li)))))
  | TestE(t1,t2,t3) ->
      let t1ex = expand_term t1 in
      let t2ex = expand_term t2 in
      let t3ex = expand_term t3 in
      (fun context ->
	t1ex (fun t1i ->
          (* Some trivial simplifications *)
          if Terms.is_true t1i then t2ex context else
  ...
  | ResE(b, t) ->
      ...
      (fun context -> Terms.oproc_from_desc (Restr(b, tex context)))
  | EventAbortE(f) ->
	 ...
	 breaking the protocol. *)
      (fun context -> Terms.oproc_from_desc (EventAbort f))
...
and expand_oprocess cur_array p =
	...
	| ((bl, def_list, t, p1)::rest_l) ->
		...
		if bl != [] || def_list != [] then
		  expand_find_list (fun rest_l' ->
		    next_f ((bl, def_list, final_pseudo_expand t,
  ...
  | Output((c,tl),t2,p) ->
	...
	Terms.oproc_from_desc (Output((c,tl),t2, expand_process cur_array p))
  | Let(pat,t,p1,p2) ->
	...
	Terms.oproc_from_desc (Let(pat, t, expand_oprocess cur_array p1, 
				   expand_oprocess cur_array p2))
  | EventP(t,p) ->
      if need_expand t then
	begin
	...
	Terms.oproc_from_desc (EventP(t, expand_oprocess cur_array p))
