...
let rec apply_not_red t =
  ...
  | FunApp(f,l) ->
    { t_desc = FunApp(f, List.map apply_not_red l);
      t_type = t.t_type;
      t_occ = t.t_occ;
...
let rec apply_red t (restr,proba,redl,redr) =
  match t.t_desc with
	...
	with NoMatch ->
	  Terms.cleanup();
	  { t_desc = FunApp(f, List.map (fun t' -> apply_red t' (restr, proba, redl, redr)) l);
	    t_type = t.t_type;
	    t_occ = t.t_occ;
	    t_loc = Parsing_helper.dummy_ext }
      end
...
let rec apply_reds simp_facts t =
  let t = reduce simp_facts t in
  apply_reds_on_reduced simp_facts t

and apply_reds_on_reduced simp_facts t =
  reduced := false;
...
let rec add_fact dep_info ((subst2, facts, elsefind) as simp_facts) fact =
  ...
  match fact'.t_desc with
	...
	match t1.t_desc with
	  Var(b,l) -> 
	    let t1' = { t_desc = Var(b, List.map (reduce simp_facts) l);
			t_type = t1.t_type;
			t_occ = Terms.new_occ();
			t_loc = Parsing_helper.dummy_ext }
	    in
...
let simplify_term dep_info keep_tuple ((subst2, facts, elsefind) as simp_facts) t = 
  let t' = reduce simp_facts t in
  let t'' = apply_reds_on_reduced simp_facts t' in
  let t''' = 
...
let rec match_term2 next_f simp_facts bl t t' =
  match t.t_desc with
	...
	match v.link with
	  NoLink -> Terms.link v (TLink t')
	| TLink t -> if not (simp_equal_terms simp_facts t t') then raise NoMatch
      end;
      next_f ()
  | Var(v,l) ->
      begin
...
let check_compatible_main b args pp cur_array simp_facts def_node_opt =
  let rec get_lcp l1 l2 = 
    ...
    | (t::l1',b2::l2') ->
	begin
	  match reduce simp_facts t with
...
let rec check_compatible2_deflist cur_array simp_facts old_def_list def_list =
  ...
  let old_def_list = List.map (fun (b,l) -> (b, List.map (reduce simp_facts) l)) old_def_list in
  let def_list = List.map (fun (b,l) -> (b, List.map (reduce simp_facts) l)) def_list in
	...
	| (dep_infoi::dep_info_l),((bl, def_list, otheruses_list, t, p1)::l) ->
		...
		(reduced_def_list (!def_node_opt) 
		 ...
		 and substitute M for i *)
	      ...
	      let subst = ref [] in
	      List.iter (fun b -> 
		let b_im = reduce tf' (Terms.term_from_binder b) in
...
let rec guess_by_matching next_f t t' = 
  match t.t_desc with
    Var (v,[]) -> 
    (* Check that types match *)
      if t'.t_type != v.btype then
	...
	match t'.t_desc with
	  FunApp(f',[t1';t2']) when f == f' ->
            begin
              try
                Terms.auto_cleanup (fun () ->
	          guess_by_matching (fun () -> guess_by_matching next_f t2 t2') t1 t1')
              with NoMatch ->
                guess_by_matching (fun () -> guess_by_matching next_f t2 t1') t1 t2'
            end
	| _ -> next_f()
      end
  | FunApp(f,l) ->
...
let guess_by_matching_same_root next_f simp_facts t t' = 
  guess_by_matching_same_root next_f t (reduce simp_facts t') 
