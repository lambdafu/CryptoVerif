...
type source_t = SSimplify | SMergeArrays | SMergeBranches

let source = ref SSimplify
...
let merge_var next_f map b b' =
  if b == b' then
    ...
    if (not ar_b) && (not ar_b') then
      next_f ((b,b')::map)
    else if (!source) != SMergeArrays (* MergeArrays does not advise other Merges *) then
      begin
...
let store_arrays_to_normal f =
  ...
  if not r then
    ...
    begin
      if (!source) == SSimplify then
	Transform.advise := Terms.add_eq MergeBranches (!Transform.advise);
      if (!source) == SMergeBranches then
	Parsing_helper.internal_error "merge_branches should not call store_arrays_to_normal";
      if (!source) == SMergeArrays then
	Parsing_helper.internal_error "merge_arrays should not accumulate advice";	
	...
	try
	  ...
	  in
	    ...
	    if List.length bl != List.length target_bl then
	      raise Failed;
	    let subst = List.map2 (fun b b' -> (b, Terms.term_from_binder b')) bl target_bl in
...
let add_advice (merge_type, cur_array, all_branches_var_list, _, _) = 
  (* TO DO If I want to give an explicit location for variable creations, 
  ...
  | FindE(l0,t3,find_info) -> 
	...
	begin
	  try
		...
		merges_to_do := (MergeFindCond(t, [t3;t2]), 
	      ...
	      collect_merges_find_cond cur_array t2;
	      if not r then
		collect_merges_find_cond cur_array t1
		  ) l0
	  with Contradiction ->
	    ()
	end
	...
	begin
	  try
	    ...
	    if not r then
	      List.iter (fun (_,_,t1,_) -> collect_merges_find_cond cur_array t1) l0
	  with Contradiction ->
	    ()
	end
