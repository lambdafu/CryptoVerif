...
let rec instan_time = function
  ...
  | (Cst _ | Count _ | Zero | Card _ ) as x -> x
...
let rec add_def_vars current_node def_vars_accu seen_refs (b,l) =
  if (List.for_all (check_non_nested [] [b]) l) &&
	...
	Terms.add_binderref br def_vars_accu) def_vars_above_def;
      ...
      (* recursive call for subterms *)
      List.iter (add_def_vars_subterms current_node def_vars_accu seen_refs) l
    end

and add_def_vars_subterms current_node def_vars_accu seen_refs t =
  match t.t_desc with
    Var(b,l) ->
  ...
  | FunApp(_,l) ->
      List.iter (add_def_vars_subterms current_node def_vars_accu seen_refs) l
  | _ -> 
      Parsing_helper.internal_error "If/let/find/new unexpected in add_def_vars_subterms"
...
TO DO (1) Activate advice? (See comments and display of "Manual advice:" below)
...
(2) The dependency analysis tends to be too sensitive to the syntax.
	...
	begin
	  if (!tmp_bad_dep) then
	    begin
	      if (!to_advise) != [] then
		begin
		  (* TO DO Uncomment next line to activate advice
  ...
  | Find(l0,p2,dn) ->
	 ...
	 in
	   ...
	   List.iter (fun br -> try 
	     add_def_vars (get_node_for_find_branch (!dn) bl) def_vars_accu seen_refs br
	   with Contradiction -> ()
...
let rec add_facts current_node fact_accu seen_refs (b,l) =
  ...
  if (List.for_all (check_non_nested [] [b]) l) &&
	...
	add_facts current_node fact_accu seen_refs (b', List.map (Terms.subst bindex l) l')
      ...
      (* recursive call for subterms *)
      List.iter (add_facts_subterms current_node fact_accu seen_refs) l
    end
      
and add_facts_subterms current_node fact_accu seen_refs t =
  match t.t_desc with
    Var(b,l) ->
      if not (Terms.is_repl b) then
...
let facts_from_defined current_node bl def_list =
  ...
  let seen_refs = ref [] in
  List.iter (add_facts current_node fact_accu seen_refs) def_list;
  !fact_accu


let add_def_vars_with_subterms bl def_vars_accu seen_refs br =
  add_def_vars None def_vars_accu seen_refs br
(*
  let subterms = ref [] in
...
let rec check_compatible lcp b pp def_node_opt is_comp = function
    Nil -> (false, false)
  | Par(p1,p2) ->
      let (has_b1, has_pp1) = check_compatible lcp b pp def_node_opt is_comp p1 in
      let (has_b2, has_pp2) = check_compatible lcp b pp def_node_opt is_comp p2 in
      if (has_b1 && has_pp2) || (has_b2 && has_pp1) then
	is_comp := true;
      (has_b1 || has_b2, has_pp1 || has_pp2)
  | (Repl(b',p) as p') ->
      if lcp <= 0 then
	(* When lcp <= 0, we have is_comp as soon as b is defined in p and pp occurs in p,
           and this can be tested very efficiently using definition nodes *)
	let (has_b, has_pp) =
	  match def_node_opt with
	    None -> check_compatible (lcp-1) b pp def_node_opt is_comp p
	  | Some pp_node ->
	...
	let (has_b, has_pp) = check_compatible (lcp-1) b pp def_node_opt is_comp p in
	if (b' == b) && has_pp then
	  is_comp := true;
	(has_b || (b' == b), has_pp)
  | Input(_,pat, p) ->
      let (has_b, has_pp) = check_compatibleo lcp b pp def_node_opt is_comp p in
      let has_b2 = Terms.occurs_in_pat b pat in
      if has_b2 && has_pp then
	is_comp := true;
      (has_b || has_b2, has_pp)

and check_compatibleo lcp b pp def_node_opt is_comp = function
    Yield -> (false, false)
  | Restr(b',p) ->
      let (has_b, has_pp) = check_compatibleo lcp b pp def_node_opt is_comp p in
      if (b' == b) && has_pp then
	is_comp := true;
      (has_b || (b' == b), has_pp)
  | Test(_, p1, p2) -> 
      let (has_b1, has_pp1) = check_compatibleo lcp b pp def_node_opt is_comp p1 in
      let (has_b2, has_pp2) = check_compatibleo lcp b pp def_node_opt is_comp p2 in
      (has_b1 || has_b2, has_pp1 || has_pp2)
  | Find(l0, p2, _) ->
      let (has_b2, has_pp2) = check_compatibleo lcp b pp def_node_opt is_comp p2 in
  ...
  | Output(_,_,p) ->
      check_compatible lcp b pp def_node_opt is_comp p 
  | EventP(_,p) ->
      check_compatibleo lcp b pp def_node_opt is_comp p 
...
let check_compatible_main b args pp cur_array simp_facts def_node_opt =
  ...
  let is_comp = ref false in
  ignore (check_compatible lcp b pp def_node_opt is_comp (!whole_game));
  !is_comp 
...
let is_compatible simp_facts (b,args) (b',args') =
  ...
  let args_skip' = Terms.skip (l'-min) args' in
  (not (List.for_all2 (simp_equal_terms simp_facts) args_skip args_skip')) ||
...
and simplify_oprocess cur_array dep_info true_facts p =
  ...
  | Find(l0, p2, def_node_opt) ->
	...
	| (dep_infoi::dep_info_l),((bl, def_list, otheruses_list, t, p1)::l) ->
	    ...
	    try
	      ...
	      let facts_def_list = facts_from_defined (get_node_for_find_branch (!def_node_opt) bl) bl def_list in
		...
		let def_vars_accu = ref def_list' in
		let seen_refs = ref [] in
		List.iter (add_def_vars (get_node_for_find_branch (!def_node_opt) bl) def_vars_accu seen_refs) def_list';
  ...
  | Let(pat, t, p1, p2) ->
	...
	[dep_info_in; dep_info_else] ->
	  ...
	  let p2' = simplify_oprocess cur_array dep_info_else true_facts p2 in
	  simplify_let dep_info dep_info_in cur_array true_facts' p1 p2' t' pat
      |	[dep_info_in] -> 
	  let t' = simplify_term dep_info (is_pat_tuple pat) true_facts t in
	  simplify_let dep_info dep_info_in cur_array true_facts' p1 Yield t' pat
...
and simplify_let dep_info dep_info_in cur_array true_facts ptrue pfalse t' = function
    (PatVar b) as pat -> 
	...
	with 
	  Not_found -> 
		...
		if (ptrue' == Yield) && (pfalse == Yield) &&
		  (not (let vars_pat = Terms.vars_from_pat [] pat in
		        (List.exists has_array_ref vars_pat) ||
		        (List.exists Transform.occurs_in_queries vars_pat))) then
		  begin
		    Transform.changed := true;
		    Yield
		  end
		else
		  Let(pat, t', ptrue', pfalse)
	      with Contradiction ->
		Transform.changed := true;
		pfalse
	    end
