...
let rec apply_red try_no_var t redl redr =
  match t.t_desc with
	...
	with NoMatch ->
	  Terms.cleanup();
	  Terms.build_term2 t (FunApp(f, List.map (fun t' -> apply_red try_no_var t' redl redr) l))
      end
  | _ -> t
...
let rec apply_eq_reds try_no_var t =
  match t.t_desc with
  ...
  | FunApp(f,[_;_]) when f.f_eq_theories != NoEq && f.f_eq_theories != Commut ->
	...
	t
  | FunApp(f,l) ->
      Terms.build_term2 t (FunApp(f, List.map (apply_eq_reds try_no_var) l))      
  | _ -> t
...
let rec apply_collision reduce_rec try_no_var t ((restr, forall, redl, proba, redr) as coll) =
  match t.t_desc with
    FunApp(f,l) ->
      begin
	try
	...
	with NoMatch ->
	  Terms.cleanup();
	  Terms.build_term2 t (FunApp(f, List.map (fun t' -> apply_collision reduce_rec try_no_var t' coll) l))
      end
  | _ -> t
  ...
  List.iter (fun t0 ->
	...
	(* When f.f_cat == LetEqual, t itself cannot be reduced by link,
	   ...
	   However, it is not clear what "simp_facts" should really be...
         *)
	let (t1, t1', lhs_reduced, rhs_reduced) = 
	  match t'.t_desc with
	    Var _ | ReplIndex _ ->
	      reduced_subst := false;
	      let t1 = apply_subst1 t link in
	      let lhs_reduced = !reduced_subst in
	      reduced_subst := false;	      
	      let t1' = apply_subst1 t' link in
	      (t1,t1',lhs_reduced,!reduced_subst)
	  | FunApp _ ->
	      reduced_subst := false;
	      let t1 = apply_subst1 t link in
	      let lhs_reduced = !reduced_subst in
	      if lhs_reduced then
		(t1, t', lhs_reduced, false)
	      else
		begin
		  reduced := false;
		  let simp_facts_tmp = (link :: (!subst2''), facts, elsefind) in
		  let t1' = apply_reds simp_facts_tmp t' in
		  (t1, t1', lhs_reduced, !reduced)
		end
	...
	in
	if lhs_reduced (* || (rhs_reduced && (match orient_eq t1 t1' with
	  None -> true
	| Some(t2,t2') -> (t1 != t2) || (t1' != t2'))) *)
	then
	  begin
	...
	else if rhs_reduced then
	  begin
	    if not (Terms.equal_terms t1 t1') then
	      subst2'' := (Terms.build_term_type Settings.t_bool (FunApp(f,[t1; t1']))) :: (!subst2'')
	  end
	else
	  subst2'' := t0 :: (!subst2'')
    | _ -> Parsing_helper.internal_error "substitutions should be Equal or LetEqual terms"
