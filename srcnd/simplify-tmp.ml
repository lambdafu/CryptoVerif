...
let rec apply_not_red simp_facts t =
  ...
  | FunApp(f,[t1;t2]) when f.f_cat == Equal && (try ignore(unify_terms simp_facts t1 t2); true with NoMatch -> false) ->
      reduced := true;
      Terms.make_true()
  | FunApp(f,[t1;t2]) when f.f_cat == Diff && (try ignore(unify_terms simp_facts t1 t2); true with NoMatch -> false) ->
      reduced := true;
      Terms.make_false()
  ...
  | Find(l0, p2, def_node_opt) ->
	...
	| (dep_infoi::dep_info_l),((bl, def_list, otheruses_list, t, p1)::l) ->
	    ...
	    try
	      ...
	      let facts_def_list = facts_from_defined (!def_node_opt) bl def_list in
...
(*  print_string "Trying to prove ";
  Display.display_query (QEvent(t1,t2));*)
