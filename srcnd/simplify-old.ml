...
(* TO DO On demand substitutions 

let on_demand_subst subst f t =
  f t'  where t' is a possible image of t
	  If f raises NoMatch, try another choice... 
...
let apply_statement t (vl,t_state) =
  match t_state.t_desc with
    FunApp(f, [t1;t2]) when f == Settings.f_equal ->
      apply_red t (t1,t2)
  | FunApp(f, [t1;t2]) when f == Settings.f_diff ->
      apply_red (apply_red (apply_red (apply_red t 
...
let is_tuple t =
  match t.t_desc with
    FunApp(f, _) when f.f_cat == Tuple -> true
  | _ -> false
...
let rec fully_defined t =
  match t.t_desc with
    FunApp(f,l) -> List.for_all fully_defined l
  | Var(b,l) -> Terms.is_restr b
  | _ -> false

let rec add_fact ((subst, subst2, facts) as simp_facts) fact'' =
  ...
  | FunApp(f, [{t_desc = Var(b,l)};t2]) when f == Settings.f_equal &&
    ...
    (not (Terms.is_restr b)) ->
      subst_simplify simp_facts (b,t2)
  ...
  | FunApp(f,[{ t_desc = Var(b1,l1) }; {t_desc = Var(b2,l2)}]) when
    ...
    b1 != b2 ->
      simp_facts
      (* TO DO add probability *)
  | FunApp(f,[{ t_desc = Var(b1,l1) }; t2]) when
    f == Settings.f_equal && (Terms.is_restr b1) && (Terms.is_large b1.btype) &&
    (not (Terms.refers_to b1 t2)) && (fully_defined t2) ->
      raise Contradiction
      (* TO DO add probability *)
  | FunApp(f,[{ t_desc = Var(b1,l1) }; t2]) when
    f == Settings.f_diff && (Terms.is_restr b1) && (Terms.is_large b1.btype) &&
    (not (Terms.refers_to b1 t2)) && (fully_defined t2) ->
      simp_facts
      (* TO DO add probability *)
  | FunApp(f,[t2; { t_desc = Var(b1,l1) }]) when
    f == Settings.f_equal && (Terms.is_restr b1) && (Terms.is_large b1.btype) &&
    (not (Terms.refers_to b1 t2)) && (fully_defined t2) ->
      raise Contradiction
      (* TO DO add probability *)
  ...
  | _ -> 
      ...
      if is_true fact'' then simp_facts else
      (subst, subst2, fact''::facts)

(* OLD VERSION faster on some examples, but more likely not to terminate

and subst_simplify (subst, subst2, facts) link =
  let subst' = List.map (fun (b,t) -> (b,apply_reds(Terms.subst2 [link] t))) subst in
  let subst2' = List.map (fun (t,t') -> (apply_reds(Terms.subst2 [link] t),apply_reds(Terms.subst2 [link] t'))) subst2 in
  ...
  simplif_add_list (link :: subst',subst2'',[]) (not_subst2_facts @ facts)
...
and subst_simplify (subst, subst2, facts) link =
  ...
  List.iter (fun (t,t') ->
    let (b,_) = link in
    if Terms.refers_to b t || Terms.refers_to b t' then 
      not_subst2_facts := (Terms.make_equal t t') :: (!not_subst2_facts)
    else
      subst2'' := (t,t') :: (!subst2'')
    ) subst2;
  simplif_add_list (link :: subst',!subst2'',[]) ((!not_subst2_facts) @ facts)
...
let simplify_term keep_tuple ((subst, subst2, facts) as simp_facts) t = 
  ...
  let t''' = 
    if t''.t_type == Settings.t_bool then
      begin
	try
	  ignore(add_fact simp_facts t'');
	  try
	    ignore(add_fact simp_facts (Terms.make_not t''));
	    simplify_term_rec t''
	  with Contradiction ->
	    Terms.make_true()
	with Contradiction ->
  ...
  if !Settings.minimal_simplifications then
    begin
      if is_true t''' || is_false t''' || (not (Terms.equal_terms t' t''')) ||
...
let rec simplify_cterm true_facts t =
  match t.t_desc with
    TestE(t1, t2, t3) ->
      begin
      let t1' = simplify_term false true_facts t1 in
      try
  ...
  | _ -> simplify_term false true_facts t
...
let rec simplify_process true_facts = function
  ...
  | Find(l0, p2) ->
	...
	| ((bl, def_list, t, p1)::l) ->
	    ...
	    try
	      ...
	      simplify_find true_facts' l' bl def_list t' p1 
	    with Contradiction ->
	      Transform.changed := true;
	      l'
      in
  ...
  | Input(t, pat, p) ->
      Input(simplify_term false true_facts t, simplify_pat true_facts pat, 
	    simplify_process true_facts p)
  | Output(t1,t2,p) ->
      Output(simplify_term false true_facts t1, simplify_term false true_facts t2,
	     simplify_process true_facts p)
