...
let rec orient t1 t2 = 
  ...
  | (FunApp _, FunApp _) ->
      Terms.count_var t1 > Terms.count_var t2
  | _ -> false
  ...
  | FunApp(f,[t1;t2]) when f.f_cat == Equal ->
	...
	| (ACUN(prod, neut) | Group(prod, _, neut) | CommutGroup(prod, _, neut)) as eq_th -> 
	    begin
	      ...
	      let l1' = List.map (try_no_var simp_facts) l1 in
	      let l2' = List.map (try_no_var simp_facts) l2 in
...
and subst_simplify2 dep_info (subst2, facts, elsefind) link =
  ...
  let rec apply_link = function
	...
	(* Reduce the LHS of the equality t = t' *)
        reduced_subst := false;
        let t1 = apply_subst1 (try_no_var ((!subst2'') @ rest, facts, elsefind)) t link in
	let (reduced_lhs, t1) = 
	  if !reduced_subst then (true, t1) else
          match t1.t_desc with
            Var _ | ReplIndex _ ->
	      (false, t1)
          | FunApp _ ->
	      ...
	      let t1 = apply_eq_statements_and_collisions_subterms_once (reduce_rec simp_facts_tmp) (try_no_var simp_facts_tmp) t1 in
	      (!reduced, t1)
	...
	let (reduced, t1') = 
	  if !reduced_subst then (true,t1') else
	      ...
	      reduced := false;
	      let simp_facts_tmp = (link :: (!subst2'') @ rest, facts, elsefind) in
	      (* Using "rest" in simp_facts_tmp may cause a loop! 
		 When rest contains C[t2] = C[t2'] and the added link is
		 t2' = t2, C[t2] reduces to C[t2'] which reduces to C[t2]
		 and so on. *)
	      let t1' = apply_eq_statements_and_collisions_subterms_once (reduce_rec simp_facts_tmp) (try_no_var simp_facts_tmp) t' in
	      (reduced_lhs || (!reduced), t1')
          | _ -> Parsing_helper.internal_error "If/let/find/new not allowed in subst_simplify2"
        in
	if reduced then
