...
let rec move_a_binder put_process bl p =
  ...
  match p.i_desc with 
    Nil -> 
      ...
      Nil
  | Par(p1,p2) ->
      let r1 = List.exists (fun b -> Terms.refers_to_process b p1) bl in
      let r2 = List.exists (fun b -> Terms.refers_to_process b p2) bl in
      if r1 && r2 then
	raise Not_found
      else 
	begin
	  Settings.changed := true;
	  if r1 then
	    Par(move_a_binder put_process bl p1,p2)
	  else if r2 then
	    Par(p1, move_a_binder put_process bl p2)
	  else
	    Par(p1,p2)
	end
  | Repl(b',p) -> raise Not_found
  | Input((c,tl), pat, p) ->
      if List.exists (fun b -> (List.exists (Terms.refers_to b) tl) || (Terms.refers_to_pat b pat)) bl then
	raise Not_found
      else
	Input((c,tl), pat, move_a_bindero put_process false bl p))

and move_a_bindero put_process array_ref bl p = 
  ...
  | Restr(b',p) -> 
      Settings.changed := true;
      Restr(b', move_a_bindero put_process array_ref bl p)
  | Test(t,p1,p2) ->
      if List.exists (fun b -> Terms.refers_to b t) bl then
	put_process (Test(t,p1,p2))
      else
	begin
	  Settings.changed:= true;
	  beneficial := true;
	  Test(t, move_a_bindero put_process array_ref bl p1, move_a_bindero put_process array_ref bl p2)
	end
  | Find(l0,p,find_info) ->
      if List.exists (fun b ->
	List.exists (fun (_, def_list, t, _) ->
	(List.exists (Terms.refers_to_br b) def_list) ||
	Terms.refers_to b t) l0) bl then
	...
	begin
	  ...
	  Find(List.map (fun (bl', def_list, t, p1) ->
	    (bl', def_list, t, 
	     move_a_bindero put_process array_ref bl p1)) l0,
	       move_a_bindero put_process array_ref bl p, find_info)
	end
  | Output((c,tl),t2,p) ->
      if (List.exists (fun b ->
	(List.exists (Terms.refers_to b) tl) || (Terms.refers_to b t2)) bl) || array_ref then
	put_process (Output((c,tl),t2,p))
      else
	begin
	  try
	    let p' = move_a_binder put_process bl p in
	    Settings.changed := true;
	    Output((c,tl), t2, p')
	  with Not_found ->
	    put_process (Output((c,tl),t2,p))
  ...
  | Let(pat, t, p1, p2) ->
      if List.exists (fun b -> (Terms.refers_to b t) || (Terms.refers_to_pat b pat)) bl then
	...
	begin
	  ...
	  match pat with
	    PatVar _ -> 
	      Let(pat, t, move_a_bindero put_process array_ref bl p1, Terms.oproc_from_desc Yield)
	  | _ -> 
	      beneficial := true;
	      Let(pat, t, move_a_bindero put_process array_ref bl p1, 
		  move_a_bindero put_process array_ref bl p2)
	end
  | EventP(t,p) ->
      if List.exists (fun b -> Terms.refers_to b t) bl then
	...
	begin
	  Settings.changed := true;
	  EventP(t, move_a_bindero put_process array_ref bl p)
	end
