...
(* Simplification of let terms in pseudo-expand *)

let rec put_lets_term l br_ptrue br_pfalse =
  match l with
    [] -> br_ptrue
  | (pat, t)::lr ->
      let (brt, ptrue) = put_lets_term lr br_ptrue br_pfalse in
      match pat with
	PatVar b ->
	  (brt, Terms.build_term ptrue (LetE(pat, t, ptrue, None)))
      | _ ->
	  let (brf, pfalse_opt) = br_pfalse in
	  (brt + brf, Terms.build_term ptrue (LetE(pat, t, ptrue, pfalse_opt)))

let simplify_let_term pat t br_ptrue ((brf, pfalse_opt) as br_pfalse) =
  try
    let (transfos, test, bind) = Terms.simplify_let_tuple pat t in
    let (brl, plet) as br_plet = put_lets_term bind br_ptrue br_pfalse in
    let test' = simplify_term test in
    if Terms.is_true test' then
      br_plet
    else
      let pfalse = Terms.get_else pfalse_opt in
      if Terms.is_false test' then 
	(brf, pfalse)
      else
	(brl + brf, Terms.build_term plet (TestE(test', plet, pfalse)))
  with Terms.Impossible -> 
    (brf, Terms.get_else pfalse_opt)
	
(* Simplification of let in expand *)

let rec put_lets l br_ptrue br_pfalse =
  match l with
    [] -> br_ptrue
  | (pat, t)::lr ->
      let (brt, ptrue) = put_lets lr br_ptrue br_pfalse in
      match pat with
	PatVar b ->
	  (brt, Terms.oproc_from_desc (Let(pat, t, ptrue, Terms.oproc_from_desc Yield)))
      | _ ->
	  let (brf, pfalse) = br_pfalse in
	  (brt + brf, Terms.oproc_from_desc (Let(pat, t, ptrue, pfalse)))


let simplify_let pat t br_ptrue ((brf, pfalse) as br_pfalse) =
  try
    let (transfos, test, bind) = Terms.simplify_let_tuple pat t in
    let (brl, plet) as br_plet = put_lets bind br_ptrue br_pfalse in
    let test' = simplify_term test in
    if Terms.is_true test' then
      br_plet
    else if Terms.is_false test' then 
      br_pfalse
    else
      (brl + brf, Terms.oproc_from_desc  (Test(test', plet, pfalse)))
  with Terms.Impossible -> 
    br_pfalse
...
let rec pseudo_expand_term t context = 
  ...
  | LetE(pat, t1, t2, topt) ->
	...
	pseudo_expand_pat pat (fun pati ->
	  let (br, t') = simplify_let_term pati t1i (1, t2') (1,topt') in
	  if br > 2 then
...
let rec expand_term t context = 
  ...
  | LetE(pat, t1, t2, topt) ->
	...
	expand_pat pat (fun pati ->
	  let (br, p') = simplify_let pati t1i (1,t2') (1,p3) in
	  if br > 2 then
...
and expand_oprocess cur_array p =
  ...
  | Let(pat,t,p1,p2) ->
	...
	begin
	  ...
	  expand_term t (fun ti ->
	    expand_pat pat (fun pati ->
	      let (br, p') = simplify_let pati ti (1,p1') (1,p2') in
	      if br > 2 then
