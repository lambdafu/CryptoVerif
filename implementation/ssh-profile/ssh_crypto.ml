...
let ssh_unpad = Profile.f1 "ssh_unpad" ssh_unpad
...
let symenc = Profile.f3 "symenc" symenc
...
let symdec = Profile.f3 "symdec" symdec

let mac = Crypto.mac (Cryptokit.MAC.hmac_sha1)
let check_mac = Crypto.mac_check (Cryptokit.MAC.hmac_sha1)

let mac = Profile.f2 "mac" mac
let check_mac = Profile.f3 "check_mac" check_mac
