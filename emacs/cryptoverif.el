...
(defvar cryptoverif-kw '("new" "out" "channel" "if" "then" "else" "find" "orfind" "suchthat" "fun" "param" "forall" "proba" "type" "equiv" "process" "let" "in" "query" "secret" "secret1" "const" "set" "defined" "collision" "event" "time" "yield" "abort" "otheruses" "maxlength" "length" "max" "newChannel" "inj" "define" "expand" "proof" "implementation" "get" "insert" "table" "letfun") "Cryptoverif keywords")

(defvar cryptoverif-builtin '("noninteractive" "bounded" "fixed" "large" "password" "compos" "decompos" "uniform" "commut" "manual" "computational" "unchanged" "exist" "all" "useful_change" "unique" "pred" "serial" "inverse" "random") "Cryptoverif builtins")
...
(defvar cryptoverifo-kw '("if" "then" "else" "find" "orfind" "suchthat" "fun" "param" "forall" "proba" "type" "equiv" "process" "let" "in" "query" "secret" "secret1" "const" "set" "defined" "collision" "event" "time" "end" "abort" "otheruses" "maxlength" "length" "max" "newOracle" "inj" "foreach" "do" "return" "define" "expand" "proof" "implementation" "get" "insert" "table" "letfun") "Cryptoverif keywords")

(defvar cryptoverifo-builtin '("noninteractive" "bounded" "fixed" "large" "password" "compos" "decompos" "uniform" "commut" "manual" "computational" "unchanged" "exist" "all" "useful_change" "unique" "pred" "serial" "inverse" "random") "Cryptoverif builtins")
